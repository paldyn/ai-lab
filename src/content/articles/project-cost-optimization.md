...
draft: false
---
[지난 글](/articles/project-deploying-llm)에서 FastAPI, Docker, Prometheus를 조합해 LLM 서비스를 프로덕션에 배포하는 방법을 살펴봤다. 이번에는 배포된 서비스를 **더 저렴하게** 운영하는 전략을 다룬다. LLM API 비용은 단순해 보이지만 조금만 파고들면 80% 이상 절감이 가능한 구조가 숨어있다. 프롬프트 캐싱부터 모델 라우팅, 배치 API, 토큰 절약 기법까지 실전에서 검증된 여섯 가지 전략을 하나씩 짚는다.
