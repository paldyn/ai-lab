...
draft: false
---
[지난 글](/articles/nlp-text-generation)에서 언어 모델이 텍스트를 생성하는 원리를 살펴봤다. 이번에는 생성된 텍스트를 이해하는 핵심 과제 중 하나인 **지시 해소**(Coreference Resolution)를 다룬다. "김철수가 회사에서 그의 동료를 만났다. 그는 매우 기뻤다"라는 문장에서 "그의"와 "그"가 모두 "김철수"를 가리킨다는 것을 파악하는 기술이다. 이 문제를 해결하지 못하면 문서 요약, QA, 정보 추출 등 모든 상위 NLP 태스크의 품질이 저하된다.
