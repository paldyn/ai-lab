...
그 논문은 앞에서 잘라도 되는 임베딩을 만드는 법에 대한 것이다. 그러면 그렇게 만들지 않은 임베딩을 앞에서 자르면 어떻게 되는가. 이 글은 그 질문 하나를 재는 글이다.
...
여기서 우리가 붙잡을 것은 뒷부분이 아니라 앞부분이다. "앞 $$m$$차원을 그냥 잘라 써도 된다"는 성질이 MRL 학습으로 만들어지는 것인가, 아니면 임베딩이 원래 어느 정도 갖고 있는 것인가.
...
`FF-2048`은 MRL 없이 보통 방식으로 학습한 2048차원 모델이다. 즉 `Rand. FS`는 MRL로 학습하지 않은 모델을 앞에서 그냥 자른 것이고, 이 글이 재려는 것과 같은 조작이다. ImageNet 1-NN 정확도(%)로 논문이 보고한 값은 이렇다.
...
이 표를 **유지율**, 곧 전체 차원의 점수 대비 줄인 차원의 점수가 몇 %인지로 다시 읽으면 주장이 선명해진다. 그냥 자른 쪽은 2048차원 대비 256에서 92.4%, 128에서 85.6%, 64에서 70.1%, 32에서는 46.2%밖에 남지 않는다. MRL은 같은 자리에서 99.5% / 99.4% / 98.9% / 97.9%다. 그냥 자르면 64차원부터 3할이 날아간다는 것이 논문이 그린 그림이다.
...
에러는 나지 않는다. 처음 돌렸을 때 드러난 유일한 신호는 MRL 모델 쪽 인코딩이 유독 느리다는 것이었고, 그게 아니었으면 못 봤을 것이다. 그래서 두 mpnet 모델 모두 `max_seq_length = 128`로 명시해 맞췄다. 기준 모델인 MiniLM은 기본값(256)을 그대로 둔다 — 그쪽은 [실험대](/articles/lab-retrieval-testbed)의 공개된 값과 맞는지 확인하는 데 쓰기 때문이다.
...
세 가지가 보인다. 첫째, 그냥 자르는 것은 생각만큼 무너지지 않는다. 그리고 무너지는 자리를 정하는 것은 차원 수가 아니라 **남긴 비율**, 곧 전체 차원 가운데 남긴 차원의 몫이다. MRL로 학습하지 않은 두 모델의 절단 유지율을 남긴 비율로 세워 보면 거의 겹친다.
...
판정은 **부트스트랩**으로 냈다 — 질의 300개를 복원 추출로 2,000번 다시 뽑아 차이의 평균이 떨어지는 95% 구간을 구하고, 그 구간이 0을 품으면 동률이다.

첫 표에서 앞 $$m$$차원이 무작위로 고른 $$m$$차원을 이긴 자리는 17번 중 한 번, matryoshka 모델의 64차원(+0.0287, 구간 [+0.0056, +0.0519])뿐이다. MRL로 학습하지 않은 두 모델은 열한 자리 모두 동률이고, mpnet-base-nli의 32·16차원에서는 앞쪽이 오히려 무작위보다 낮게 나왔다(−0.0194, −0.0133). 구간이 0을 품어 동률로 판정됐을 뿐이다.

둘째 표도 같은 자리를 가리킨다. 두 mpnet의 절단 유지율이 갈린 곳은 64차원 하나로, MRL 쪽이 55.3% 대 47.1%, +8.2%p(구간 [+1.3, +14.6])다. 나머지 다섯 자리는 동률이다. 64는 학습 스크립트의 중첩 차원 가운데 맨 아래이고, 그보다 위인 512·256·128은 훈련 지점인데도 차이가 구간 안에 들었다. 훈련에 안 쓰인 32·16도 동률이다.
...
첫 줄과 마지막 줄은 살아남았고 가운데 셋은 이 체크포인트에서 성립하지 않았다. 다시 말하지만 이것이 MRL이라는 방법에 대한 판정은 아니다. 우리가 돌린 것은 논문 저자의 모델이 아니라 커뮤니티가 같은 손실로 학습한 별개의 체크포인트이고, 그 학습이 얼마나 잘 됐는지는 우리가 보증할 수 없다. 이 표가 말하는 것은 「MRL 손실로 학습했다」는 라벨만으로 절단 내성을 가정하면 안 된다는 것이다.
...
- MRL 모델이 논문의 모델이 아니다. 우리가 쓴 것은 sentence-transformers 커뮤니티가 MRL 손실로 학습한 mpnet이고, 논문의 ResNet50·BERT 실험과 학습 규모·데이터가 전혀 다르다. MRL이 잘 되게 학습됐는지 자체를 우리가 보증하지 못한다 — 위 결과에서 MRL 쪽이 기대만큼 안 나오면 방법의 문제인지 이 체크포인트의 문제인지 가를 수 없다.
