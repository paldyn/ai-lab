...
행이 한 줄뿐인 $$1 \times n$$ 행렬도 있고 **행**이라 부릅니다. 이 글에서는 열만 쓰지만, 뒤에서 곱을 정의할 때 "$$A$$ 의 한 행"을 떼어 보는 일이 계속 나옵니다.
...
**뺄셈은 따로 정의할 것이 없습니다.** $$-1$$ 을 곱한 것을 더하면 됩니다.

$$A - B = A + (-1)B$$

수를 다룰 때 뺄셈을 "음수를 더하는 것"으로 읽었던 것이 그대로 옵니다. 괄호를 푸는 규칙도 같습니다. 성분마다 따로 하는 계산이라 **수의 분배법칙이 자리마다 한 번씩 적용되는 셈**이기 때문입니다.

$$k(A+B) = kA + kB, \qquad (k+l)A = kA + lA$$

**덧셈과 스칼라배까지는 새로 익힐 것이 없다**는 뜻입니다. 수에서 하던 대로 하면 맞습니다.
...
![AB는 (19 22 / 43 50), BA는 (23 34 / 31 46)로 네 자리가 모두 다른 그림](/assets/posts/math-basics-matrix-notation-order.svg)
