...
draft: false
---
[지난 글](/articles/cv-diffusion-basics)에서 확산 모델의 수학적 원리인 순방향·역방향 과정과 노이즈 스케줄러를 다뤘다. 이번 글에서는 이 원리를 **잠재 공간**(Latent Space)으로 이동시켜 실용적으로 만든 **Stable Diffusion**의 전체 파이프라인과 실전 사용법을 다룬다.

## LDM의 핵심 혁신

픽셀 공간에서 직접 확산을 수행하면 512×512 이미지도 매우 무겁다. **LDM**(Latent Diffusion Model)의 핵심 아이디어는 **VAE로 이미지를 먼저 압축**한 뒤, 작은 잠재 공간에서 확산을 수행하는 것이다. 512×512 픽셀 이미지는 VAE 인코더를 통해 64×64×4 잠재 벡터로 8배 압축된다. 확산 과정은 이 잠재 공간에서 이루어져 메모리와 연산량이 크게 줄어든다.
