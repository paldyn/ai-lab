...
draft: false
---
[지난 글](/articles/nlp-machine-translation)에서 한 언어를 다른 언어로 변환하는 기계 번역을 살펴봤다. 번역도 사실 "텍스트 생성"의 특수한 형태다. 이번에는 더 일반적인 의미의 **텍스트 생성(Text Generation)** 자체를 깊이 다룬다. "오늘 날씨가..."라는 프롬프트에서 시작해 모델이 어떻게 자연스러운 문장을 이어 쓰는지, 어떤 디코딩 전략이 어떤 상황에 적합한지, 그리고 품질을 어떻게 측정하는지를 코드와 함께 완전히 해설한다.
