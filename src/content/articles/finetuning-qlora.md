...
**페이지드 옵티마이저**(Paged Optimizer)는 NVIDIA의 통합 메모리(Unified Memory) 기능을 활용해 이 문제를 해결한다. GPU VRAM이 부족해질 때 옵티마이저 상태(Adam의 1차·2차 모멘텀)를 자동으로 CPU RAM으로 **페이지 스왑**한다. 마치 운영체제가 RAM 부족 시 디스크 스왑을 쓰는 것과 같은 원리다. 이렇게 하면 OOM 에러 없이 학습이 계속 진행된다. 물론 스왑이 빈번하게 발생하면 속도가 느려지지만, 학습이 중단되는 것보다는 훨씬 낫다.
