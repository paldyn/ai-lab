...
draft: false
---
[지난 글](/articles/ml-roc-auc)에서 분류 모델의 임계값 독립적 평가 방법을 배웠다. 이번에는 분류와 다른 방식으로 성능을 측정해야 하는 **회귀(Regression)** 모델의 평가 지표로 넘어간다. 회귀 모델이 예측한 값이 실제 값과 얼마나 가까운지를 다양한 방법으로 정량화한다.
