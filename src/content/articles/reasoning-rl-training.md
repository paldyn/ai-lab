...
draft: false
---
[지난 글](/articles/reasoning-test-time-compute)에서는 이미 훈련된 모델에 컴퓨트를 더 쓰는 방법을 다뤘다. 그런데 추론 모델의 진짜 차별점은 추론 시점의 기법이 아니라 훈련 단계에서 얻은 습관이다. 긴 사고 구간을 만들고, 중간에 "잠깐, 이건 아닌 것 같다"며 되돌아가고, 답을 쓰기 전에 스스로 검산하는 행동은 프롬프트로 지시해서 나오는 것이 아니라 훈련으로 몸에 밴 것이다. 그 훈련이 어떻게 작동하는지 알면 왜 특정 영역에서만 추론 모델이 강한지도 자연스럽게 이해된다.
