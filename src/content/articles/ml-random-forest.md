...
draft: false
---
[지난 글](/articles/ml-decision-tree)에서 결정 트리가 높은 해석 가능성을 가지지만 분산이 크고 과적합에 취약하다는 단점을 확인했다. **랜덤 포레스트**(Random Forest)는 바로 이 약점을 정면으로 해결하는 알고리즘이다. 아이디어는 단순하다. 하나의 결정 트리는 불안정하지만, 서로 다른 수백 개의 트리를 만들어 다수결로 예측하면 개별 트리의 오류가 서로 상쇄되어 훨씬 안정적이고 강건한 모델이 된다. 이것이 **앙상블(Ensemble) 학습**의 핵심이다.

## 앙상블이란? 약한 학습기를 모아 강하게
