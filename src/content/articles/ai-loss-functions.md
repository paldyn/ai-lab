...
draft: false
---
[지난 글](/articles/ai-optimizers)에서 옵티마이저가 파라미터를 어떻게 업데이트하는지 살펴봤다. 옵티마이저가 최소화하려는 목표값이 바로 **손실 함수**(Loss Function)다. "무엇을 최소화하느냐"가 모델이 학습하는 것을 결정한다. 잘못된 손실 함수를 선택하면 아무리 좋은 옵티마이저를 쓰더라도 원하는 결과를 얻을 수 없다.
...
**지난 글:** [옵티마이저 완전 정복: SGD에서 AdamW까지](/articles/ai-optimizers)
