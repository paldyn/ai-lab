...
draft: false
---
[지난 글](/articles/llm-rlhf)에서 RLHF가 보상 모델과 PPO를 거쳐 선호를 반영하는 절차와, DPO가 그 보상 모델을 대수로 지워 선호 쌍에서 곧바로 학습하는 길을 나란히 살펴봤다. 두 방법은 학습 절차가 다를 뿐 재료는 같았다 — RLHF와 DPO 모두 선호 데이터를 만드는 과정에서 인간 레이블러에 의존한다. 인간 레이블러가 응답 쌍을 비교해 어떤 것이 더 좋은지 판단하는 것이다. 그런데 이 과정에는 문제가 있다. 레이블러마다 기준이 다르고, 확장성이 낮으며, 특히 위험한 콘텐츠를 반복적으로 접해야 하는 레이블러의 정신 건강 문제도 심각하다. Anthropic은 2022년 이 문제를 근본적으로 다르게 접근하는 **Constitutional AI**(헌법적 AI, CAI)를 발표했다. AI 스스로 원칙(Constitution)에 따라 자신의 응답을 비판하고 개선하게 함으로써, 인간 레이블러의 역할을 최소화한다.
...
**지난 글:** [선호 정렬: RLHF와 DPO](/articles/llm-rlhf)
