...
draft: false
---
[지난 글](/articles/prompt-context-management)에서 긴 대화와 문서에서 컨텍스트를 효율적으로 관리하는 전략을 다뤘다. 이번 글에서는 프롬프트 자체를 어떻게 관리할지를 다룬다. 프롬프트는 소프트웨어의 "소스 코드"다. 변경 이력이 없으면 어떤 수정이 성능을 떨어뜨렸는지 파악하기 어렵고, 배포 후 문제가 생겨도 되돌리기가 힘들다. **프롬프트 버전 관리**는 이 문제를 해결하는 체계적인 접근법이다.
...
코드와 마찬가지로 프롬프트에도 **시맨틱 버전닝**(Semantic Versioning)의 `MAJOR.MINOR.PATCH` 버전 체계를 적용한다.
