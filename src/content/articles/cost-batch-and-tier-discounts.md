...
배수를 세기 전에 무엇을 세었는지부터 적는다. 출처가 둘이고 그중 하나는 2차
출처라, 검증하는 코드를 같이 싣는다.
...
확정값이라고 적혀 있는 쪽을 먼저 턴다. 문서가 쓴 50%가 문서 자신의 표와 맞는지,
2차 출처도 같은 값을 담고 있는지, 우선 처리와 리전 할증의 배수는 어떤 모양인지,
그리고 그 배수들이 겹칠 때 곱셈 순서가 결과를 바꾸는지 차례로 본다.
...
같은 JSON에 배치 말고도 배수가 둘 더 있다 — 우선 처리 모드와 리전 할증이다.
하나는 흩어지고 하나는 한 값으로 모인다.
...
캐시를 켤지 말지는 이 구간 어디에 서 있느냐로 갈린다. 경계를 먼저 식으로 구한다.
...
둘 중 어느 경계가 적용되는지도 문서가 정해 둔다. 배치 문서는 5분 캐시를 쓰지
말라고 적는다.
...
걸리는 자리가 셋이다. 요청에 든 파라미터, 같이 켤 수 없는 모드와 기능, 그리고
배치 하나의 크기와 시간이다.
...
여기까지 센 배수와 조항을 한 문장과 표 하나로 줄인다.
