...
draft: false
---
[지난 글](/articles/ml-clustering-kmeans)에서 K-평균 군집화가 데이터를 K개 그룹으로 나누는 원리를 살펴봤다. K-평균은 빠르고 간단하지만 한 가지 중요한 약점이 있다. 군집 수 K를 사전에 지정해야 한다는 것이다. 데이터를 처음 탐색하는 단계에서 K가 몇 개인지 미리 알기란 쉽지 않다. **계층적 군집화**(Hierarchical Clustering)는 이 문제를 다르게 풀어낸다. 모든 병합(또는 분리) 과정을 나무 구조로 기록해 두고, 나중에 원하는 수준에서 군집 수를 결정할 수 있다.
...
응집형 군집화의 전 과정을 시각화한 것이 **덴드로그램**(Dendrogram)이다. 트리 구조로 생각하면 이해하기 쉽다.
