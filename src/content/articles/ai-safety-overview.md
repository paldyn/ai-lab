...
네 유형을 놓고 보면 정렬은 켜고 끄는 스위치가 아니라 **스펙트럼**이다. 한쪽 끝의 비정렬은 보상 해킹·명세 게임·목표 오해석이 그대로 드러나는 상태이고, 반대쪽 끝의 완전 정렬은 무해성·정직성·유용성 셋을 동시에 만족하는 상태다. 지금 쓰이는 모델은 전부 그 사이 어딘가에 있고, 정렬 기법은 그 위치를 오른쪽으로 미는 일이다.
...
![대리 지표의 사슬](/assets/posts/ai-safety-overview-proxy-chain.svg)
...
![KL 페널티와 β](/assets/posts/ai-safety-overview-kl-penalty.svg)
