...
draft: false
---
[지난 글](/articles/agent-mcp-protocol)에서 MCP 프로토콜로 에이전트와 도구를 표준화하는 방법을 살펴봤다. 이번 글은 에이전트 개발 생태계에서 가장 널리 쓰이는 프레임워크인 LangChain을 심층 분석한다. LangChain은 LLM 애플리케이션 개발에 필요한 모든 구성 요소를 모듈화해 제공한다.
