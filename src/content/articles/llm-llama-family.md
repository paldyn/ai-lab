...
draft: false
---
[지난 글](/articles/llm-gemini-family)에서 Google Gemini의 멀티모달 네이티브 전략을 살펴봤다. GPT, Claude, Gemini 모두 API를 통해서만 접근 가능하고, 모델 자체는 공개되지 않은 상업 모델이다. 그런데 AI 생태계에는 전혀 다른 방향이 있다. 누구나 모델 가중치를 직접 다운로드해 자신의 컴퓨터에서 실행하고, 원하는 방식으로 파인튜닝할 수 있는 완전한 오픈소스 LLM. 이 혁명의 진원지가 바로 Meta의 **LLaMA 패밀리**다.
...
**다음 글:** [오픈 웨이트 도전자들: Mistral, Qwen, DeepSeek](/articles/llm-mistral-family)
