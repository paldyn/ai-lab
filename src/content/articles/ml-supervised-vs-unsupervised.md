...
draft: false
---
[지난 글](/articles/ai-loss-functions)에서 손실 함수가 학습 목표를 정하고, 그 손실 위에 항을 더하거나 절차를 바꾸는 방식으로 과적합을 억제한다는 것을 봤다. 이제 머신러닝(ML) 섹션에 본격 진입한다. ML에는 크게 두 가지 학습 패러다임이 있다. "정답이 있는가?" — 이 질문 하나가 알고리즘 선택과 데이터 수집 방식을 결정한다.
