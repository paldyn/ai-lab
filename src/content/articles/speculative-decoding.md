...
draft: false
---
[지난 글](/articles/pruning)에서 불필요한 가중치를 제거해 모델을 압축하는 프루닝을 다뤘다. 이번에는 모델 자체는 그대로 두면서 추론 속도를 극적으로 높이는 **투기적 디코딩**(Speculative Decoding)이다. LLM 추론의 근본적 병목을 우회하는 이 기법은 2~4배 속도 향상을 모델 품질 손실 없이 달성한다. GPT-4, Claude 같은 상용 서비스에서도 이미 사용하는 기술이다.
