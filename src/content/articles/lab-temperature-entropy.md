...
상태다. 표준편차가 T=1.5의 2.320에서 T=2.0의 1.331로 오히려 줄어드는 것도 같은
...
같은 표3을 이번에는 자르는 장치 쪽에서 읽는다. 개수로 자르느냐 질량으로 자르느냐에
따라 온도에 반응하는 방향이 갈린다.
...
않는 변환이다. $$p_i \propto \exp(z_i / T)$$ 에서 $$T > 0$$ 이면 $$z_i$$ 의 크기
순서가 그대로 $$p_i$$ 의 순서가 된다. 그러니 상위 50개가 누구인지는 온도와
무관하다. 온도 2.0에서든 0.2에서든 top-k=50은 정확히 같은 50개 토큰을 남긴다.

### 질량과 개수

반대로 그 50개가 얼마를 쥐고 있는지는 온도에 크게 흔들린다. T=0.2에서 1.0000,
...
어디까지가 공짜이고 어디부터 손해인지를 먼저 적고, 그다음 이 숫자가 덮지 못하는
자리를 밝힌다.
...
- 한 번의 forward에서 잰 값이다. 각 위치의 로짓은 정답 문장이 주어진 상태에서
