...
draft: false
---
[지난 글](/articles/python-for-ai)에서 AI 개발에 필요한 Python 라이브러리 생태계를 살펴봤다. 이번에는 딥러닝 연구와 실전 개발의 표준이 된 **PyTorch**의 핵심 개념을 다룬다. 텐서 생성과 조작, 자동미분, 그리고 `nn.Module`로 모델을 정의하는 방식까지 코드 중심으로 이해한다.
