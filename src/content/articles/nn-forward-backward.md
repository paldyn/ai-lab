...
draft: false
---
[지난 글](/articles/nn-activation-functions)에서 활성화 함수가 신경망에 비선형성을 부여한다는 것을 배웠다. 이제 신경망이 **어떻게 학습하는지**를 파헤칠 차례다. 신경망 학습의 핵심은 **역전파**(Backpropagation)다. 역전파가 없었다면 현대 딥러닝은 존재하지 않았을 것이다. 1986년 Rumelhart, Hinton, Williams가 발표한 이 알고리즘은 다층 신경망의 가중치를 효율적으로 학습할 수 있게 해주었다. 이번 글에서는 순전파로 예측을 만들고, 역전파로 기울기를 계산하고, 그 기울기로 파라미터를 업데이트하는 전체 과정을 수식과 코드로 명확히 이해한다.
...
**순전파**(Forward Pass)는 입력 x가 층을 통과하며 예측값을 생성하는 과정이다. 각 층에서:
