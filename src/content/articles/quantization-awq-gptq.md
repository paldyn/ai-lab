...
draft: false
---
[지난 글](/articles/quantization-gguf)에서 GGUF 포맷과 K-Quant 레벨을 살펴봤다. GGUF가 CPU 추론에 최적화된 로컬용 포맷이라면, AWQ와 GPTQ는 GPU 서버에서 높은 처리량을 유지하면서 INT4로 압축하는 데 특화된 방법이다. 같은 INT4라도 어떤 알고리즘으로 양자화하느냐에 따라 품질과 속도가 크게 달라진다. 이 글에서는 두 방법의 수학적 핵심과 실전 적용 방법을 나란히 비교한다.
