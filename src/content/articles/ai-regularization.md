...
draft: false
---
[지난 글](/articles/ai-loss-functions)에서 손실 함수가 모델의 학습 목표를 정의한다고 배웠다. 이번에는 모델이 훈련 데이터에만 지나치게 최적화되는 **과적합**(Overfitting)을 방지하는 다양한 **정규화(Regularization)** 기법을 다룬다. 과적합은 딥러닝에서 가장 흔한 문제이며, 이를 해결하지 않으면 실제 환경에서 모델 성능이 크게 저하된다.
