...
`generate_answer` 안에서 다시 스팬을 열면 그것도 자동으로 `user_request`의 자식이 된다. 부모를 인자로 넘기지 않아도 되는 이유는 현재 스팬이 실행 문맥에 담겨 있기 때문이고, 뒤에서 볼 「문맥 전파」가 전부 이 문맥이 넘어가지 못한 경계 이야기다.
...
![무작위 1%와 끝난 뒤 고르는 규칙이 각각 남기는 트레이스](/assets/posts/llmops-observability-sampling.svg)
