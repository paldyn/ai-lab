...
draft: false
---
[지난 글](/articles/rl-q-learning)에서 상태마다 행동의 가치를 적어 두는 Q-테이블에서 출발해, 그 표가 무너지는 자리를 신경망으로 넘어선 DQN까지 따라갔다. 둘은 저장하는 자리만 다를 뿐 하는 일이 같았다 — **가치를 배우고 정책은 $$\arg\max$$ 로 뽑아 쓴다.** 그리고 그 $$\arg\max$$ 가 벽을 하나 남겼다. 값을 비교하려면 행동을 하나씩 셀 수 있어야 하므로, DQN은 상태가 아무리 커도 괜찮지만 행동은 이산이어야 한다. 로봇 팔 관절을 몇 도 회전할지, 자동차 핸들을 얼마나 돌릴지 같은 **연속적인 행동**은 어떻게 학습할까? **정책 경사법(Policy Gradient)** 은 정책 자체를 신경망으로 파라미터화하고 직접 최적화하는 방법으로, 연속 행동 공간을 자연스럽게 처리하며 LLM의 RLHF 훈련에도 핵심적으로 사용된다.
