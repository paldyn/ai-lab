...
메모리 최적화는 한 번에 하나씩 적용하고 프로파일링으로 효과를 측정하는 습관이 중요하다. 다음 글에서는 이러한 기법들을 바탕으로 **파인튜닝 파이프라인을 처음부터 끝까지 만드는 프로젝트**를 진행한다. 데이터 준비부터 학습, 평가, 배포까지다.
