...
384차원을 256·128·64·32·16으로 내리며 같은 질의 집합을 다시 검색한다. 재는 것은 차원마다 남는 품질과 그때 줄어드는 메모리 둘뿐이다.
...
차원을 한 칸씩 내릴 때 잃는 몫이 어디서 커지는지를 두 코퍼스에서 따로 본다.
...
PCA로 차원을 고를 때 가장 흔히 쓰는 기준은 **누적 설명 분산이 95%가 되는 차원을 잡는다**는 규칙이다. 위에서 잰 무릎을 이 규칙이 짚어 내는지 본다.

### 누적 설명 분산

검색에서도 통할까. 같은 임베딩으로 재 봤다.
