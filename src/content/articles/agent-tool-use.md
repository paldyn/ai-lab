...
draft: false
---
[지난 글](/articles/agent-architecture)에서 에이전트 아키텍처 패턴을 살펴봤다. 어떤 아키텍처를 쓰든 에이전트의 핵심 능력은 **도구 사용**(Tool Use)이다. LLM이 외부 함수를 호출하고 그 결과를 다음 추론에 활용하는 이 메커니즘을 완전히 이해해야 실용적인 에이전트를 만들 수 있다.
