...
draft: false
---
[지난 글](/articles/vector-db-pgvector)에서 PostgreSQL 확장인 pgvector로 벡터 데이터를 저장하고 유사도 검색을 수행하는 방법을 살펴봤다. 이제 그 벡터 검색 기술이 LLM과 만나는 지점, 즉 **RAG**(Retrieval-Augmented Generation)를 다룬다. RAG는 "검색 증강 생성"이라고 번역되는데, LLM이 답변을 생성할 때 외부 지식을 실시간으로 검색해 참조하도록 하는 기법이다. 2020년 Facebook AI Research(현 Meta AI)의 논문 "Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks"에서 처음 제안됐고, 지금은 실무 AI 시스템의 표준 아키텍처로 자리잡았다.
