...
draft: false
---
[지난 글](/articles/rag-vs-finetuning)에서 RAG와 파인튜닝의 차이를 심층적으로 비교했다. 이번에는 한 발 물러서서 LLM을 특정 목적에 맞게 최적화하는 **세 가지 핵심 전략 전체**를 비교한다. 프롬프트 엔지니어링, RAG, 파인튜닝은 서로 다른 문제를 해결하며, 실제 프로젝트에서 어떤 순서로 접근해야 하는지 명확한 기준이 필요하다.
...
**다음 글:** [파인튜닝 완전 정복: 사전 학습 모델을 내 데이터로 특화하는 방법](/articles/finetuning-overview)
