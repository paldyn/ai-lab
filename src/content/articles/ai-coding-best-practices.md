...
윤리 쪽에는 세 가지가 더 있다. 생성된 코드가 어디서 온 것인지 모르니 **라이선스**를 확인한다 — 유명한 라이브러리의 코드를 통째로 재현한 결과가 나올 수 있다. 팀 안팎에 AI가 만든 코드임을 **공시**한다 — 뒤의 PR 서술 절에서 다룬다. 그리고 도구가 제공하는 프라이버시 모드나 코드 유출 방지 설정을 켜 두어 회사 코드가 밖으로 나가는 경로를 줄인다.
