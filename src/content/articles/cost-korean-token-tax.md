...
아래 「미러의 병합 표현」 절에 있고 막히지 않은 환경에서는 아무 일도 하지 않는다.
...
`tax.py`가 아홉 개를 차례로 로드해 네 말뭉치를 인코딩하고, 같은 문단을 스무 번 이어
붙인 것으로 처리량을 다섯 번 재 중앙값을 남긴다. 게이트된 둘은 마지막에 따로 열어
실패 메시지를 그대로 찍는다 — 표에 추정치를 채우지 않기 위해서다.
...
이 글은 청구되는 양만 쟀다. 토큰이 적게 드는 것과 그 모델이 한국어를 잘하는 것은
다른 문제이고, 여기 있는 아홉 줄은 품질에 대해 아무 말도 하지 않는다.
...
그리고 게이트된 둘이 빠져 있다. Llama와 Gemma 계열이 표에 없으므로 "모든 주요
