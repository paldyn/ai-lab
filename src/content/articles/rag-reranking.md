...
draft: false
---
[지난 글](/articles/rag-retrieval-strategies)에서 BM25·벡터·하이브리드 검색이라는 세 가지 검색 전략을 완전히 파악했다. 이제 한 단계 더 나아가, 1차 검색으로 뽑아낸 수십 개의 후보 문서를 **더 정확한 기준으로 다시 정렬**하는 **리랭킹**(Reranking)을 다룬다. 리랭킹을 추가하면 동일한 검색 인덱스를 사용하더라도 LLM에 전달되는 컨텍스트 품질이 눈에 띄게 올라가고, 결과적으로 최종 답변의 정확성도 함께 높아진다.
...
**다음 글:** [질문을 바꿔 다시 검색하기: 재작성·분해·멀티홉](/articles/rag-query-rewriting)
