...
답이 이상하다. 문맥상 중요한 단어가 아니라 맨 앞의 첫 토큰을 본다. 그것도 조금이
...
무엇으로 쟀는지, 어텐션 행렬을 어느 경로로 꺼냈는지, 그리고 나온 출력을 한자리에 둔다.
...
평균한 것이다. 어텐션 행렬의 한 행은 합이 1이므로 이 값은 그대로 "몇 %를 가져갔나"로
읽힌다. 0.5면 절반이다.
...
배수구가 어느 층에서 열리는지, 그리고 0번 자리에 앉은 토큰이 무엇이냐에 따라
크기가 달라지는지를 차례로 본다.
...
0층에서 2층까지 0번 토큰은 평범한 토큰이다. 영어 12.7토큰짜리 문장에서 한 토큰의
...
거의 같은 값이 나온다. 내용에 반응하는 층과 위치에만 반응하는 층이 갈리는 지점이다.

### 위치와 내용

3층부터의 값이 문장 내용과 무관하게 일정하다면, 애초에 0번 토큰이 무엇인지가
상관없을 수도 있다. 확인해 봤다.
...
처음 출력의 EN-KO 차이는 언어의 성질처럼 보였다. 그 값이 실제로 무엇을 재고 있었는지를
대조 둘로 갈라 본다.
...
생긴 값이었다. 토크나이저가 한국어를 잘게 쪼개는 문제([/articles/tokenizer-bpe](/articles/tokenizer-bpe)가
...
배수구가 열리고 닫히는 자리를 숫자로 적고, 그다음 이 실험대가 덮지 못하는 곳을 밝힌다.
...
- 어텐션 가중치는 인과가 아니다. 0번 토큰을 많이 본다는 것이 0번 토큰의 정보를
  ...
  [편향과 설명](/articles/ai-bias-fairness)이 이 구별을 다룬다.

### 자기검사

첫째, 앞서 적은 bfloat16 사고다. 초안 스크립트는 `dtype`을 지정하지 않았고
