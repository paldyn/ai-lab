...
draft: false
---
[지난 글](/articles/anthropic-sdk)에서 Anthropic SDK로 Claude API를 호출하고 툴 사용, 스트리밍, 비전을 다뤘다. 이번에는 현재 AI 개발 생태계에서 가장 넓게 쓰이는 **OpenAI Python SDK**를 정면으로 해부한다. `openai` 패키지 하나로 Chat Completions부터 Function Calling, 임베딩, 이미지 입력, Structured Outputs, 비동기 클라이언트, 배치 API까지 전부 커버한다.
