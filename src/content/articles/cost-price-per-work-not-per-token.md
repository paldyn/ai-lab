...
막혀 있어(아래 「막힌 출처」 참조) 검증할 수 없는 숫자를 표에 박지 않았다.
...
①번 표에서 견디는 배수가 1.35를 넘는 짝이 일곱이고 가장 큰 것이 2.04배다 —
A.X-4.0-Light의 토크나이저로 세는 쪽은 Mistral-7B-Instruct-v0.3의 토크나이저로
세는 쪽보다 단가가 2.04배 비싸도 한국어 청구서가 같다. o200k_base와 cl100k_base
사이도 1.40배라, 자/토큰이 다른 둘을 견줄 때는 단가표만으로 답이 안 나온다.
...
> 차이 나면 단가가 2.04배 비싼 모델이 한국어에서 본전이다.
...
쪽 컴퓨터에서는 열릴 가능성이 높다. 추정치로 채우지 않고 비워 둔다. 남은 두 벤더
사이에서도 순위 역전은 관찰되지 않았고, 관찰될 수 있는 조건(서로 다른 토크나이저)을
확인할 방법이 없다.
...
Anthropic과 Google은 자기 토크나이저를 공개하지 않는다. 토큰 수를 정확히 세려면
`countTokens` 계열 API를 불러야 하고 그건 키가 필요하다. 그래서 실비 표의 자/토큰은
공개된 넷에서 온 대역이고, 두 벤더의 실제 값이 이 대역 밖일 수 있다. 이 표는
「이 벤더의 한국어 자/토큰은 정확히 얼마다」를 말하지 못하고, 공개된 토크나이저로 잰
1.12~2.27자를 대입하면 청구서가 그 범위에 들어온다까지만 말한다.

그래서 이 글이 확정적으로 말하는 것은 「4자 환산은 한국어에서 맞지 않는다」와
「Anthropic 문서가 적어 둔 30%」 둘이고, 벤더 자신의 숫자로 닫혀 있는 것은 뒤의
것뿐이다.
...
말뭉치는 우리 글이다. 자/토큰 1.12\~2.27은 이 사이트의 AI·수학 글에서 나온
값이고 한글 음절 비율이 43.3%다. 순수 한국어 산문만 다루는 곳이라면 자/토큰이
더 낮아져(앞 글의 순수 한국어 측정에서는 0.89\~2.67) 배수가 더 커진다.

작업량도 가정이다. 1만 건 × 2,000자 입력 + 400자 출력은 우리가 정한 값이다.
