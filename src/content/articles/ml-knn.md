...
draft: false
---
[지난 글](/articles/ml-logistic-regression)에서 로지스틱 회귀가 시그모이드로 선형 결정 경계를 만드는 원리를 배웠다. 이번에는 전혀 다른 접근법인 **K-최근접 이웃**(K-Nearest Neighbors, KNN)을 다룬다. KNN은 "비슷한 것끼리는 비슷한 레이블을 가진다"는 간단한 가정 하나로 동작한다. 수학적 최적화도, 파라미터 학습도 없다. 새 데이터가 들어오면 훈련 데이터 중 가장 가까운 K개를 찾아 다수결로 분류한다.
