...
draft: false
---
[지난 글](/articles/ml-linear-regression)에서 같은 선형 결합에 무엇을 씌우느냐가 회귀와 분류를 가른다는 것을 봤다. 이번에는 전혀 다른 접근법인 **K-최근접 이웃**(K-Nearest Neighbors, KNN)을 다룬다. KNN은 "비슷한 것끼리는 비슷한 레이블을 가진다"는 간단한 가정 하나로 동작한다. 수학적 최적화도, 파라미터 학습도 없다. 새 데이터가 들어오면 훈련 데이터 중 가장 가까운 K개를 찾아 다수결로 분류한다.
