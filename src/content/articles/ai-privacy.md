...
draft: false
---
[지난 글](/articles/ai-bias-fairness)에서 모델의 결정이 어느 집단에 치우치는지를 숫자로 재고, SHAP·LIME 같은 기법으로 그 결정의 근거를 열어 봤다. 그런데 모델을 열어 보면 학습 데이터가 함께 비친다 — 모델이 데이터를 너무 잘 기억하면 그 자체가 프라이버시 문제가 된다. AI 시스템은 어떻게 개인정보를 유출하고, 어떻게 보호할 수 있을까.
