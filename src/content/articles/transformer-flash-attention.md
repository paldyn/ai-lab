...
draft: false
---
[지난 글](/articles/transformer-rotary)에서 RoPE가 회전을 통해 어텐션 내적에 위치 정보를 심는 방법을 살펴봤다. 이번에는 그 어텐션 연산 자체를 근본적으로 가속하는 방법인 **FlashAttention**을 다룬다. 2022년 Tri Dao 등이 발표한 FlashAttention은 알고리즘적 혁신 없이 오직 메모리 계층 구조 활용만으로 어텐션을 최대 7.6배 빠르게, 메모리는 O(N) 수준으로 줄였다. 현재 주요 LLM 학습·추론의 사실상 필수 구성요소다.
