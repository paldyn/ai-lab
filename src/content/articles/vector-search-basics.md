...
draft: false
---
[지난 글](/articles/prompt-versioning)에서 프롬프트를 버전으로 박아 두고 평가로 재는 한 바퀴를 만들었다. 그런데 그 순환을 아무리 잘 돌려도 모델이 모르는 것은 나오지 않는다 — 사내 규정도 어제 바뀐 가격표도 프롬프트를 어떻게 고쳐서 얻을 수 있는 것이 아니고, 필요한 문서를 밖에서 찾아 넣어 주는 수밖에 없다. 문제는 그 찾는 방식이다. 낱말이 겹치는 문서를 고르는 식으로는 「환불 규정」과 「돈을 돌려받으려면」이 같은 것을 묻는다는 사실을 놓친다. 이번 글부터는 문장을 벡터로 바꿔 뜻이 가까운 것끼리 모으는 **벡터 검색**(Vector Search)을 깊이 파헤친다. RAG(검색 증강 생성), 시맨틱 검색, 추천 시스템의 공통 근간이 바로 벡터 검색이기 때문이다.
...
**지난 글:** [프롬프트를 코드처럼 다루기: 버전 관리와 평가](/articles/prompt-versioning)
