...
draft: false
---
[지난 글](/articles/prompt-evaluation)에서 프롬프트 평가 파이프라인을 갖추면 LLM 시스템의 품질을 데이터로 통제할 수 있음을 확인했다. 이번 글부터는 그 시스템의 중요한 축인 **벡터 검색**(Vector Search)을 깊이 파헤친다. RAG(검색 증강 생성), 시맨틱 검색, 추천 시스템의 공통 근간이 바로 벡터 검색이기 때문이다.
