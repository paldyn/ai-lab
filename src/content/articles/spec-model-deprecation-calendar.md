...
모델을 고를 때 품질과 단가는 본다. 그런데 그 모델이 얼마나 오래 살아 있는가는
...
555개 중 하나는 날짜가 아니다. `sample_spec` 항목의
`deprecation_date`에는 `"date when the model becomes deprecated in the format
YYYY-MM-DD"`라는 설명문이 들어 있다. 이 JSON은 맨 앞에 스키마 예시를 한 항목으로
...
달력의 38.8%는 이미 과거다. 554개 중 215개가 2026-09-02보다 앞선 날짜다.
...
대다수 모델에는 이 필드가 아예 없다. chat 모델 2,390개 중 날짜를 가진 것은
...
핵심은 **폐기**(deprecated)와 **은퇴**(retired)가 다른 날짜라는 것이다. 폐기는
...
Anthropic 모델 26개는 전부 출력 단가가 입력 단가의 정확히 5.0배다. 비율이 모든
...
그리고 이전이 늘 싸지지는 않는다. 일곱 경로 중 하나는 청구액이 4.00배가 된다.
...
아니라 벤더가 지정한 것이므로 협상할 자리도 없다.

### 토크나이저 세대

절감 경로에서는 토크나이저가 절감의 3분의 1을 먹는다. Opus 4.1 → Opus 4.8은
단가가 $15에서 $5로 0.33배가 되지만, 가격 문서가 "Claude 4.7 이후 모델은 새
토크나이저를 쓰며 같은 텍스트에 약 30% 더 많은 토큰을 만든다"고 적어 두었다.
곱하면 실제 청구액은 0.43배다. 표에 적힌 −67%가 청구서에서는 −57%가 된다.
앞 글들이 잰
...
| 달력을 자동화한다 | `deprecation_date`를 은퇴일로 읽는다. 60일을 더하지 않는다 | 대조 15개 전부가 은퇴일 |
| 알림을 만든다 | 날짜 모양을 정규식으로 먼저 거른다 | 555개 중 1개가 설명문 |
| 필드가 비어 있다 | "오래 산다"로 읽지 않고 미확인으로 둔다 | chat 2,390개 중 2,022개가 빈칸 |
| 대체 모델이 지정됐다 | 품질 회귀보다 청구액 배수를 먼저 계산한다 | 일곱 경로 중 하나가 4.00배 |
...
통보 기간 아홉 건도 표본이 작다. 최소 60일은 문서가 약속한 값과 정확히 같은데,
...
다르다"고 적어 두었으니 0.43배는 점 추정이 아니라 대푯값이다. 한국어 비중이
높은 워크로드에서 이 값이 어떻게 움직이는지는 재지 못했다.

청구액 배수도 단가만 본 값이다. 캐시 배수, 배치 할인, 레지던시 배수는 이전
...
오늘 문서를 열어 보니 인상이 취소되고 $2/$10이 표준가라고 적혀 있다. 표에
