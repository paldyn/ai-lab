...
이 글은 그 실험이 낸 결론을 스스로 감사하는 실험이다. 코퍼스 크기를 흔들면서 같은 곡선을 다시 그려, 무릎이 고정된 숫자인지 코퍼스 크기의 함수인지를 가른다. PCA가 무엇을 하는지는 [PCA: 고차원 데이터를 압축하는 차원 축소](/articles/ml-pca)가 맡는다.
...
우리가 알고 싶은 것은 그 코퍼스에서 384차원이 낸 값의 몇 %가 남는가다. 오른쪽 세 열의 **남는 품질**이 그것이고, 여기서부터가 실험이다.
