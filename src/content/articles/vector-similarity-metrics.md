...
draft: false
---
[지난 글](/articles/vector-search-basics)에서 벡터 검색의 전체 파이프라인을 살펴봤다. 임베딩 모델이 텍스트를 고차원 벡터로 변환하고, 유사한 의미를 가진 벡터는 공간에서 가깝게 위치한다는 개념을 이해했다. 그렇다면 "가깝다"는 것을 정확히 어떻게 측정할까? 그것이 바로 **유사도 지표(Similarity Metric)** 의 역할이다.
