...
draft: false
---
[지난 글](/articles/cnn-semantic-segmentation)에서 모든 픽셀에 클래스를 붙이는 시맨틱 분할부터 같은 클래스의 물체까지 개별로 가르는 인스턴스 분할까지, 이미지를 픽셀 단위로 읽어내는 방법을 살펴봤다. 거기서 입력은 이미 `(3, H, W)` 격자로 가지런히 들어와 있었다. 이번에는 다루는 데이터가 텍스트로 바뀌는데, 텍스트는 그런 격자로 오지 않는다. 어떤 모델에 넣든 간에 원시 텍스트를 그대로 사용하는 경우는 거의 없다. 실제 데이터는 HTML 태그, 특수문자, 오타, 중복 공백, 이모지, 구어체 표현 등으로 가득하다. 이 "노이즈"를 걷어내고 모델이 학습할 수 있는 형태로 변환하는 과정이 **텍스트 전처리**(Text Preprocessing)다. 데이터 과학자들이 흔히 "데이터 전처리에 80%의 시간을 쓴다"고 말하는 것은 과장이 아니다. 좋은 전처리가 모델 성능을 결정하는 경우가 많다.
...
**지난 글:** [세그멘테이션: 픽셀 분류에서 개별 인스턴스까지](/articles/cnn-semantic-segmentation)
