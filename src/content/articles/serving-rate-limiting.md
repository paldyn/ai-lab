...
draft: false
---
[지난 글](/articles/serving-streaming)에서 SSE 스트리밍으로 LLM 응답을 실시간 전송하는 방법을 다뤘다. API 서버를 외부에 노출하면 반드시 마주치는 문제가 있다. 악의적인 사용자의 남용, 단일 클라이언트의 과도한 요청, 갑작스러운 트래픽 폭주가 전체 서비스를 다운시킬 수 있다. Rate Limiting은 이를 방어하는 필수 메커니즘이다.
