...
draft: false
---
[지난 글](/articles/agent-memory)에서 에이전트의 메모리 아키텍처를 살펴봤다. 이번 글에서는 에이전트가 **어떻게 계획을 세우고 실행하는지**, 즉 플래닝 전략을 다룬다. 플래닝은 에이전트 성능의 핵심이며, 같은 LLM을 사용하더라도 전략에 따라 복잡한 태스크 성공률이 크게 달라진다.
