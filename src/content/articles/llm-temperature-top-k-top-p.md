...
draft: false
---
[지난 글](/articles/llm-context-window)에서 LLM이 한 번에 처리할 수 있는 정보의 양, 즉 컨텍스트 윈도우의 구조와 한계를 살펴봤다. 이번에는 그 컨텍스트 안에서 LLM이 다음 토큰을 **어떻게 선택하는가**를 제어하는 세 가지 핵심 파라미터인 Temperature, Top-k, Top-p를 다룬다. 이 파라미터들을 이해하면 "왜 같은 프롬프트에 다른 답이 나오는가"를 정확히 설명할 수 있고, 태스크에 맞는 최적의 생성 설정을 구성할 수 있다.
