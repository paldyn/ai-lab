...
draft: false
---
[지난 글](/articles/finetuning-data-prep)에서 파인튜닝에 필요한 데이터 형식과 품질 관리 파이프라인을 완전히 파악했다. 이제 데이터가 준비됐다면 다음 단계는 **하이퍼파라미터 설정**이다. 학습률을 조금만 잘못 설정해도 모델이 발산하거나 과적합에 빠진다. 이 글에서는 LLM 파인튜닝의 핵심 하이퍼파라미터 각각의 역할과 최적값 선택 전략을 체계적으로 정리한다.
