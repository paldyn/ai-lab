...
draft: false
---
[지난 글](/articles/prompt-self-consistency)에서 다수결 앙상블로 정확도를 높이는 Self-Consistency를 살펴봤다. 이번 글에서는 LLM 프로덕션 시스템의 기반이 되는 **시스템 메시지(System Message)** 설계를 다룬다. 잘 설계된 시스템 메시지는 모델의 페르소나, 행동 경계, 출력 형식을 일관되게 제어해준다. 실제 서비스에서 사용자가 보내는 질문이 아무리 다양해도, 시스템 메시지가 탄탄하면 일관된 경험을 제공할 수 있다.
