...
draft: false
---
[지난 글](/articles/project-deploying-llm)에서 FastAPI 서버에 스트리밍과 레이트 리미팅을 붙이고, nginx 뒤에 인스턴스를 여러 개 세우고, 큐 깊이와 레이턴시를 지켜보며 LLM을 서비스로 내보내는 과정을 봤다. 그 구조는 전부 요청이 서버까지 왕복한다는 전제 위에 서 있다. GPU도, 큐도, 요청당 비용도 그 전제에서 나온다. 전제 자체를 걷어내는 선택지가 하나 있다. 서버로 보내지 않고 사용자의 기기에서 그대로 돌리는 것이다.
