...
조건을 세기 전에 무엇을 세었는지부터 적는다. 출처가 두 종류이고, 그중 하나는
2차 출처라 검증을 함께 싣는다.
...
창 옆에 적히지 않는 값 둘이다. 하나는 받을 수 있는 양의 상한이고, 하나는 창
안에서 단가가 한 번 바뀌는 자리다.
...
1M이라는 자리에 실제로 무엇이 얼마나 들어가는가다. 창을 먹는 것이 본문만이
아니고, 넘쳤을 때의 동작이 세대마다 다르며, 같은 창이 한국어에서는 다른 자릿수가 된다.
...
쓰게 된다. 앞 절의 출력 상한과 겹치는 자리이기도 하다.
...
이 표가 어디까지 말하는 값인지와, 그 값을 어디서 뽑았는지다.
