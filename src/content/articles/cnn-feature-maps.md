...
draft: false
---
[지난 글](/articles/cnn-pooling)에서 풀링이 공간 정보를 압축하고 위치 불변성을 만드는 원리를 살펴봤다. 합성곱과 풀링이 쌓이면서 만들어지는 중간 출력물—**특징 맵(Feature Map)**—이 실제로 무엇을 담고 있는지 이해하는 것은 CNN을 효과적으로 활용하기 위한 핵심이다. 특징 맵의 본질을 파악하면 전이학습 전략도 자연스럽게 따라온다.
