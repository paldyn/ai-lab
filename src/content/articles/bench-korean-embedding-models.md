...
여섯 모델을 같은 기계에 올려 두 과제를 잇달아 재는 것이 전부다. 축이 무엇이고,
그대로 복붙하면 어디까지 다시 나오는지를 먼저 못 박아 둔다.
...
같은 여섯 모델을 두 과제로 줄 세우면 줄이 서로 다르게 선다. 어긋난 자리와 그것을
만든 값이 이 절이다.
...
창이 모자란 쪽과의 차이는 크게 벌어졌지만, 창이 넉넉한 셋끼리는 이 표본에서 줄이
서지 않는다. 그 사실이 bge-m3의 값을 따지는 자리로 그대로 이어진다.
...
여기까지의 숫자를 고르는 절차로 옮긴다. 어디까지가 공짜인지를 먼저 한 줄로 적고,
그 줄에서 규칙 다섯을 뽑는다.
...
위 표가 성립하려면 지켜야 했던 것들이다. 두 자리는 실제로 틀리게 재고 나서야 알았고,
나머지는 애초에 정해 두고 들어간 값이다.
