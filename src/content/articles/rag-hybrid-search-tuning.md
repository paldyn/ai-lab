...
draft: false
---
[지난 글](/articles/context-system-prompt-design)에서 매 요청 맨 앞에 실려 가는 시스템 프롬프트를 어디까지 적을지 정했고, 이번 글부터는 그 뒤에 붙을 문서를 골라 오는 검색 쪽으로 옮긴다. 주제는 흔한 상황 하나다. 키워드 검색과 벡터 검색을 합쳐 하이브리드로 바꿨는데, 기대했던 만큼 좋아지지 않는 경우다.
