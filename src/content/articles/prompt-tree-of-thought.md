...
draft: false
---
[지난 글](/articles/prompt-chain-of-thought)에서 LLM이 단계별 추론을 생성하는 Chain-of-Thought 기법을 살펴봤다. CoT는 하나의 추론 경로를 직선으로 뻗어나가는 방식이다. 하지만 복잡한 문제는 여러 경로를 탐색하고 비교해야 한다. 이번 글에서는 2023년 Yao et al.이 제안한 **Tree-of-Thought**(ToT)를 다룬다. 추론을 트리 구조로 확장해 막힌 경로는 가지치기하고, 유망한 경로를 선택적으로 심화 탐색하는 아이디어다.
