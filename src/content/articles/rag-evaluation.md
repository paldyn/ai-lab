...
draft: false
---
[지난 글](/articles/rag-graph-rag)에서 지식 그래프를 활용한 Graph RAG를 배웠다. 다양한 RAG 기법을 익혔다면 이제 자연스럽게 드는 질문은 "어떻게 내 RAG 시스템이 잘 작동하는지 알 수 있나?"다. **RAG 평가**는 단순히 LLM의 답변 품질만 보는 것이 아니라, 검색 단계와 생성 단계를 각각 측정하고 파이프라인 전체의 병목을 파악하는 것이 핵심이다.
