...
draft: false
---
[지난 글](/articles/recsys-llm-based)에서 LLM이 추천 시스템을 어떻게 혁신하는지 살펴보았다. 이번 글부터는 AI의 또 다른 거대한 축, **강화학습(Reinforcement Learning, RL)** 을 탐구한다. 강화학습은 지도학습과 완전히 다른 패러다임이다. 정답 레이블이 없다. 에이전트가 환경과 직접 상호작용하면서 시행착오를 통해 스스로 학습한다. 바둑에서 인간 챔피언을 꺾은 AlphaGo, ChatGPT를 훈련시킨 RLHF, 자율주행 차량의 의사결정 엔진까지 — 모두 강화학습이 핵심이다.
