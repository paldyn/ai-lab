...
draft: false
---
[지난 글](/articles/rag-query-rewriting)에서 쿼리 재작성으로 검색 품질을 높이는 방법을 배웠다. 이번에는 훨씬 어려운 문제, 즉 **여러 사실을 연쇄적으로 찾아야만 답할 수 있는 복합 질문**을 다룬다. "AlphaFold를 개발한 회사의 CEO는?"이라는 질문은 단번에 검색해서 답할 수 없다. "AlphaFold 개발사"와 "그 회사의 CEO"를 연속적으로 찾아야 한다. 이처럼 **두 홉 이상의 추론이 필요한 검색**을 멀티홉 RAG(Multi-Hop RAG)라고 한다.
