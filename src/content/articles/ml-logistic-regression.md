...
draft: false
---
[지난 글](/articles/ml-linear-regression)에서 선형 회귀가 연속값 예측 문제를 풀었다. 이번에는 **분류 문제**를 다루는 **로지스틱 회귀**(Logistic Regression)를 살펴본다. 이름에 "회귀"가 붙었지만 분류 알고리즘이다. 선형 회귀의 출력을 시그모이드 함수로 변환해 확률로 만드는 것이 핵심 아이디어다.
