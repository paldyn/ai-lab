...
sourceSlug: "python-with-context"
sourceUrl: "https://techblog.paldyn.com/posts/python-with-context/"
pubDate: "2026-05-22"
syncedAt: "2026-09-08"
---

[지난 글](https://techblog.paldyn.com/posts/python-open-modes/)에서 `open()` 모드를 살펴봤다. 파일을 열었으면 반드시 닫아야 한다. 예외가 발생해도, 조기에 `return`이 실행돼도 닫혀야 한다. 이 "반드시"를 보장하는 것이 `with` 문이다. `with` 문은 파일에만 국한되지 않는다. 데이터베이스 연결, 스레드 락, 임시 디렉터리, HTTP 세션 등 **획득-사용-반환** 패턴이 필요한 모든 자원에 적용된다.
...
![with 문의 실행 흐름](https://techblog.paldyn.com/assets/posts/python-with-context-overview.svg)
...
![with 문 활용 패턴](https://techblog.paldyn.com/assets/posts/python-with-context-code.svg)
...
**지난 글:** [파일 열기 모드: open() 완전 정리](https://techblog.paldyn.com/posts/python-open-modes/)

**다음 글:** [텍스트 vs 이진 모드: 개행 문자와 인코딩의 차이](https://techblog.paldyn.com/posts/python-text-vs-binary-mode/)
