...
sourceSlug: "python-logging-module"
sourceUrl: "https://techblog.paldyn.com/posts/python-logging-module/"
pubDate: "2026-06-03"
syncedAt: "2026-09-08"
---

[지난 글](https://techblog.paldyn.com/posts/python-print-debugging-pitfalls/)에서 `print` 디버깅의 네 가지 함정을 짚으며, 그것을 한 번에 메우는 도구로 `logging`을 예고했다. 많은 사람이 `logging`을 "설정이 복잡한 print"쯤으로 오해하고 멀리한다. 하지만 핵심 개념 몇 개만 잡으면, 오히려 매번 `print`를 켰다 껐다 하는 것보다 훨씬 단순하다. 오늘은 그 개념들을 처음부터 쌓아 올린다.

## 다섯 가지 레벨

`logging`의 출발점은 레벨이다. 모든 로그 메시지에는 심각도가 매겨지고, 로거는 정해진 기준선 이상의 메시지만 통과시킨다. 이 한 가지 장치가 `print` 디버깅의 "전부 아니면 전무" 문제를 해결한다.

![로그 레벨의 위계](https://techblog.paldyn.com/assets/posts/python-logging-levels.svg)
...
![로그 한 줄이 흘러가는 길](https://techblog.paldyn.com/assets/posts/python-logging-pipeline.svg)
...
**지난 글:** [print 디버깅의 함정과 졸업](https://techblog.paldyn.com/posts/python-print-debugging-pitfalls/)

**다음 글:** [logging 설정: 로거 계층과 dictConfig](https://techblog.paldyn.com/posts/python-logging-config/)
