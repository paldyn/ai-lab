...
sourceSlug: "python-pytest-basics"
sourceUrl: "https://techblog.paldyn.com/posts/python-pytest-basics/"
pubDate: "2026-06-02"
syncedAt: "2026-09-08"
---

[지난 글](https://techblog.paldyn.com/posts/python-unittest-basics/)에서 표준 라이브러리 `unittest`로 테스트의 기본기를 익혔다. 클래스를 만들고 `assertEqual` 같은 전용 메서드를 쓰는 방식이었는데, 막상 써 보면 검증 한 줄을 적기 위해 외워야 할 메서드가 꽤 많다고 느껴진다. 파이썬 생태계에서 사실상 표준으로 자리 잡은 `pytest`는 이 부담을 거의 없앤다. 평범한 함수와 파이썬 내장 `assert` 문만으로 테스트를 쓰게 해 주면서도, 실패했을 때는 `unittest`보다 더 친절한 정보를 보여 준다.

## 함수 하나가 테스트 하나

pytest에서는 클래스도, 상속도 필요 없다. `test_`로 시작하는 **그냥 함수**를 만들고 그 안에서 `assert`로 검증하면 끝이다. 먼저 `pip install pytest`로 설치한다.

![같은 검증, 두 가지 방식](https://techblog.paldyn.com/assets/posts/python-pytest-basics-vs-unittest.svg)
...
![pytest 실행 흐름](https://techblog.paldyn.com/assets/posts/python-pytest-basics-run-flow.svg)
...
**지난 글:** [unittest 기초: 표준 라이브러리로 시작하는 테스트](https://techblog.paldyn.com/posts/python-unittest-basics/)

**다음 글:** [pytest fixture: 테스트 준비물을 우아하게 주입하기](https://techblog.paldyn.com/posts/python-pytest-fixtures/)
