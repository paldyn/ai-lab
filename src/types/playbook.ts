...
export interface ModelInfo {
  ...
  apiId: string | null;
  /**
   * **어떤 상황에서 이 모델을 쓰나** — 벤더가 제 페이지에 적어 둔 말.
   *
   * **주장이 아니라 등록부입니다.** 이름·`apiId`와 같은 급이라 제자리에 출처를 답니다 —
   * 썩는 **수**(컨텍스트 창·단가)만 주장으로 서고 확인 로그가 나이를 붙입니다. 이것은
   * 수가 아니라 벤더가 그 모델을 어디에 놓았는지이고, 값이 틀리는 것이 아니라 **자리가
   * 바뀌는** 종류라 분기 구조 점검이 맡습니다(`PLAYBOOK-ROUTINE.md`).
   *
   * **못 본 것은 `null`입니다.** 벤더가 그 말을 안 하는 모델이 실제로 넷 있습니다 —
   * 은퇴했거나(legacy), 다른 회사 제품에 얹혀 돌아 그 회사 문서에만 있거나, 카드에
   * 자랑만 적혀 있는 경우입니다. 지어내지 않습니다.
   *
   * `text`는 원문을 우리 어투로 옮긴 한 줄이고 `url`은 **그 문장이 실린 페이지**입니다 —
   * 이름을 본 페이지(`sourceUrl`)와 다를 수 있어 따로 답니다.
   */
   ...
   *
   * 오늘 이것을 가진 것은 **Anthropic 일곱**뿐입니다. Anthropic은 등급마다 손그림
 ...
 */
/**
 * 아낌 축의 질문 넷. **세션이 지나는 시간** 순서입니다 —
 * 열기 전에 정해 두는 것 → 어느 모델로 → 일을 시킬 때 → 끊는 자리.
 */
export type SaveGroupId = 'load' | 'model' | 'feed' | 'cut';

/**
 * 잘 씀 축의 질문 넷. **어긋남을 어디서 잡나**이고 차례는 **고치는 값이 커지는
 * 순서**입니다 — 한 번 박아 두면 이후 전부에 걸리는 층 → 실행 0회에서 막는 요청
 * 한 통 → 이미 돌고 있는 것을 짚어 고치기 → 다 나온 뒤에야 걸리는 판정.
 *
 * **아낌 축과 다른 넷을 씁니다**(2026-09-17). 처음에는 두 축이 같은 넷을 썼는데,
 * 그 넷은 세션의 시간으로 짠 것이라 아낌에는 잘 맞고(20·10·16·17) **잘 씀에서는
 * 서른둘 중 스물넷이 `feed` 하나에 몰렸습니다**(75%). 제품 아홉 중 여섯이 「잘 쓰기」
 * 절에 묶음을 하나만 세웠고, 묶음이 하나면 묶은 뜻이 없습니다. 지금은 가장 큰 칸이
 * 34%이고 팁이 둘 이상인 제품 여덟이 전부 두 묶음 이상 섭니다.
 */
export type WellGroupId = 'standing' | 'brief' | 'steer' | 'verdict';

export type TipGroupId = SaveGroupId | WellGroupId;
