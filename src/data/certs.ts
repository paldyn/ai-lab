...
/**
 * 별 다섯 개로 그리는 눈금. 난이도와 취업 활용도가 함께 씁니다.
 * **우리 판단이지 시행처가 매긴 값이 아닙니다.**
 *
 * **반 칸을 씁니다.** 눈금이 다섯뿐이면 「입문이지만 구현 비중이 있는」 자리가
 * 갈 곳이 없어 위나 아래로 밀려납니다. AI-900이 그 자리라 난이도가 1.5입니다.
 *
 * ## 난이도 — 응시자격·시험 형식·권장 경력
 *
 ...
 * 보는 시험이 한 칸에 묶여 목록에서 갈리지 않습니다.
 *
 * ## 취업 활용도 — 국내 채용·인사 제도에서 실제로 쓰이는가
 *
 * **법적 지위를 먼저 보고 그다음에 등급을 봅니다.** 느낌으로 매기면 「AWS가
 * 요즘 인기니까」 같은 것이 들어오는데, 지위는 확인할 수 있는 사실입니다.
 *
 * 5 국가기술자격 — 공공기관 가산점·학점은행 학점이 제도로 명문화
 * 4 국가공인 민간자격 — 공인 번호가 있고 국내 공고에 상시 등장
 * 3 벤더 공식 자격 + 국내 채용에 흔한 스택, 어소시에이트 이상
 * 2 벤더 자격이지만 국내 언급이 드물거나 파운데이셔널 등급
 * 1 채용에서 사실상 쓰이지 않음
 *
 * 반 칸은 **같은 지위 안의 등급 차이**에 씁니다. 국가공인 민간자격 넷 가운데
 * 전문가 등급(ADP·SQLP)이 4.5이고 준전문가·개발자 등급이 4입니다. 벤더 쪽에서는
 * 어소시에이트인데 국내 공고 언급을 확인하지 못한 셋이 2.5입니다.
 *
 * 근거는 `employmentBasis`에 한 줄로 적습니다. 채용 공고 건수처럼 검색 시점에
 * 따라 변하는 수치는 쓰지 않습니다 — 확인한 날 하루만 맞는 값입니다.
 */
export type CertRating = 1 | 1.5 | 2 | 2.5 | 3 | 3.5 | 4 | 4.5 | 5;
 ...
 */
/** 환불 규정 한 줄. 「언제까지면 얼마」입니다. */
export interface CertRefund {
  /** 언제까지인가. 시행처 문장의 시점 표현을 그대로 옮깁니다. */
  when: string;
  /** 얼마를 돌려주는가. 「전액」·「50%」·「불가」 셋 중 하나로 적습니다. */
  rate: string;
  /** 그 줄에만 붙는 단서. 없으면 비웁니다. */
  note?: string;
}
...
export interface CertExamSession {
  ...
  documentTo?: string;
  /**
   * 날짜가 아닌 것만. 「온라인·오프라인」, 「금·토 이틀」처럼 칸으로 못 담는 것입니다.
   *
   * **날짜를 여기 문장으로 적지 않습니다.** 수험표 발급일이나 서류 제출 기간은
   * 위에 칸이 따로 있고 표에서도 각자의 열에 섭니다 — 메모에 적으면 회차 이름
   * 밑에 잔글씨로 깔려 훑을 수가 없습니다.
   */
  note?: string;
}
...
export interface Cert {
  ...
  /** 국내 채용·인사 제도에서 얼마나 쓰이는가. 우리 판단입니다. */
  employment: CertRating;
  /** 그 숫자를 매긴 근거 한 줄. 법적 지위가 1차 근거입니다. */
  ...
  fee?: string;
  /**
   * 환불 규정. **문장이 아니라 표로 담습니다.**
   *
   * 시행처의 환불 안내는 「언제까지면 얼마」의 되풀이라 문장으로 적으면 한 문단에
   * 시점 셋과 비율 셋이 뒤엉킵니다 — 정작 필요한 물음은 「지금 취소하면 얼마
   * 돌려받나」 하나인데 그 답을 찾으려면 문단을 통째로 읽어야 했습니다.
   *
   * 시점 표현은 시행처 문장을 그대로 옮깁니다(「접수 마감 이후 ~ 시행 5일 전
   * 17:59:59」). 우리가 날짜로 계산해 적으면 회차마다 틀립니다.
   */
  refund?: CertRefund[];
  /** 표에 담기지 않는 환불 이야기 — 예외 사유, 처리 기준 시각 같은 것. */
  refundNote?: string;
  validity?: string;
  prerequisite?: string;
...
export const certs: Cert[] = [
  {
    ...
    employment: 5,
    employmentBasis: '국가기술자격 기사 등급이라 제도상 우대가 명문화돼 있습니다.',
    whatItMeasures:
    ...
    schedule: [
      {
        ...
        examDate: '2026-04-04',
        previewFrom: '2026-04-17',
        previewTo: '2026-04-21',
        resultDate: '2026-04-24',
        documentFrom: '2026-04-24',
        documentTo: '2026-05-06',
      },
      {
        round: '제12회 실기',
        applyFrom: '2026-05-18',
        applyTo: '2026-05-22',
        ticketDate: '2026-06-05',
        examDate: '2026-06-20',
        previewFrom: '2026-07-03',
        previewTo: '2026-07-07',
      ...
      },
      {
        ...
        applyTo: '2026-10-30',
        ticketDate: '2026-11-13',
        examDate: '2026-11-28',
        previewFrom: '2026-12-11',
        previewTo: '2026-12-15',
        resultDate: '2026-12-18',
      },
    ],
    scheduleUrl: 'https://www.dataq.or.kr/www/accept/schedule.do',
    fee: '필기 17,800원, 실기 40,800원. 원서 제출 후 2시간 이내에 수수료를 결제하지 않으면 접수가 자동 취소된다.',
    refund: [
      { when: '접수 기간 마감일 17:59:59까지', rate: '전액' },
      { when: '접수 마감 이후 ~ 검정 시행 5일 전 17:59:59', rate: '50%' },
      { when: '검정 시행 5일 전 18:00:00 이후', rate: '불가' },
    ],
    validity:
  ...
  },
  {
    ...
    employment: 4,
    employmentBasis: '자격기본법 근거의 국가공인 민간자격이고 준전문가 등급입니다.',
    whatItMeasures:
    ...
    schedule: [
      {
        ...
        applyTo: '2026-01-09',
        ticketDate: '2026-01-23',
        examDate: '2026-02-07',
        previewFrom: '2026-02-27',
        previewTo: '2026-03-03',
      ...
      },
      {
        ...
        applyTo: '2026-04-17',
        ticketDate: '2026-05-01',
        examDate: '2026-05-17',
        previewFrom: '2026-05-29',
        previewTo: '2026-06-02',
      ...
      },
      {
        ...
        examDate: '2026-08-08',
        previewFrom: '2026-08-21',
        previewTo: '2026-08-25',
      ...
      },
      {
        ...
        applyTo: '2026-10-02',
        ticketDate: '2026-10-16',
        examDate: '2026-10-31',
        previewFrom: '2026-11-13',
        previewTo: '2026-11-17',
        resultDate: '2026-11-20',
      },
    ],
    scheduleUrl: 'https://www.dataq.or.kr/www/accept/schedule.do',
    fee:
      '50,000원. 공식 검정수수료 표의 "데이터 분석 준전문가(ADsP)" 행이다(같은 표에서 ADP는 필기 80,000원 · 실기 70,000원). 납부방법은 "신용카드, ' +
      '계좌이체"이고 원서를 제출하고 2시간 이내에 수수료를 납부하지 않으면 접수가 자동 취소된다(접수 마감일에는 17:59:59까지 결제가 완료되어야 한다).',
    refund: [
      { when: '접수 기간 마감일 17:59:59까지', rate: '전액' },
      { when: '접수 마감 이후 ~ 검정 시행 5일 전 17:59:59', rate: '50%' },
      {
        when: '검정 시행 5일 전 18:00:00 이후',
        rate: '불가',
        note: '시험일이 토요일이면 그 주 월요일, 일요일이면 그 주 화요일 18:00:00이 기준이다',
      },
    ],
    validity:
    ...
    notes:
      ...
      '변경은 접수 기간 중에만 가능하다.\n' +
      '- 지필 시험이고 답안은 종이 답안지에 적는다. 시행처가 \'PBT\'라는 명칭을 쓰지는 않지만 응시자 유의사항이 검정색 컴퓨터용 사인펜이나 볼펜만 ' +
      '쓰게 하고 채점은 OCR 스캐너 판독을 따른다고 밝히며, 시험을 마치면 문제지와 답안지를 모두 감독관에게 제출하게 한다. 같은 페이지의 시험시간 표에 ' +
      'ADsP는 10:00~11:30(90분)으로 적혀 있다.\n' +
      '- 자격증은 상장형(국가공인자격)이라 홈페이지를 통해 즉시 발급된다. 시행처가 안내하는 발급 수수료 6,200원은 카드형인 국가기술자격 ' +
      '빅데이터분석기사에만 붙고 상장형에는 금액이 제시돼 있지 않다. 재발급 비용도 페이지에 언급이 없다.\n' +
      '- 회차별 통계는 없지만 연도별 통계는 있다. 시행처 FAQ가 통계 소재를 한국직업능력연구원 민간자격정보서비스(pqi.or.kr)로 지정하고, 그곳 ' +
      '「데이터분석」(등록번호 2014-1112) 상세 화면의 준전문가(공인) 등급 행이 ADsP다. 최근 세 해 합격률이 60%대 중후반으로, 같은 표의 ' +
      '전문가(공인) 등급(=ADP)이 한 자릿수 초반인 것과 크게 갈린다. pqi는 이 수치가 「자격발급기관에서 직접 입력」한 값이라는 단서를 단다.',
    unknowns: [
  ...
  },
  {
    ...
    employment: 4.5,
    employmentBasis: '같은 국가공인 자격의 전문가 등급이고 학점은행 30학점입니다.',
    whatItMeasures:
    ...
    schedule: [
      ...
      },
      {
        ...
        applyTo: '2026-04-03',
        ticketDate: '2026-04-17',
        examDate: '2026-05-02',
        previewFrom: '2026-05-22',
        previewTo: '2026-05-26',
        resultDate: '2026-05-29',
        documentFrom: '2026-05-29',
        documentTo: '2026-06-08',
      },
      {
        round: '제37회 필기',
        applyFrom: '2026-07-06',
        ...
        ticketDate: '2026-07-24',
        examDate: '2026-08-08',
        previewFrom: '2026-08-28',
        previewTo: '2026-09-01',
      ...
      },
      {
        ...
        applyTo: '2026-09-18',
        ticketDate: '2026-10-02',
        examDate: '2026-10-17',
        previewFrom: '2026-11-06',
        previewTo: '2026-11-10',
        resultDate: '2026-11-13',
        documentFrom: '2026-11-13',
        documentTo: '2026-11-20',
      },
    ],
    scheduleUrl: 'https://www.dataq.or.kr/www/accept/schedule.do',
    fee:
      '필기 80,000원, 실기 70,000원(각각 따로 접수·납부). 납부는 신용카드·계좌이체이며 원서 제출 후 2시간 안에 납부하지 않으면 접수가 자동 취소된다 — 단, 접수 ' +
      '마감일에는 17:59:59까지 결제가 완료되어야 한다.',
    refund: [
      { when: '접수 기간 마감일 17:59:59까지', rate: '전액' },
      { when: '접수 마감 이후 ~ 검정 시행 5일 전 17:59:59', rate: '50%' },
      {
        when: '검정 시행 5일 전 18:00:00 이후',
        rate: '불가',
        note: '시험일이 토요일이면 그 주 월요일 18:00:00, 일요일이면 그 주 화요일 18:00:00이 기준',
      },
    ],
    refundNote:
      '접수한 시험을 다음 회차로 연기하는 제도는 없다. 본인·가족의 사고·질병·사망, 격리 대상 전염병 확진, 군인의 외출 금지, 천재지변에 한해 증빙을 붙여 시험일로부터 30일 ' +
      '이내 신청하면 100% 환불하는 예외 규정이 있다.',
    validity:
  ...
  },
  {
    ...
    employment: 4,
    employmentBasis: '국가공인 SQL 자격의 개발자 등급이고 학점은행 6학점입니다.',
    whatItMeasures:
    ...
    schedule: [
      {
        ...
        examDate: '2026-03-07',
        previewFrom: '2026-03-20',
        previewTo: '2026-03-24',
      ...
      },
      {
        ...
        examDate: '2026-05-31',
        previewFrom: '2026-06-12',
        previewTo: '2026-06-16',
      ...
      },
      {
        ...
        examDate: '2026-08-22',
        previewFrom: '2026-09-04',
        previewTo: '2026-09-08',
      ...
      },
      {
        ...
        applyTo: '2026-10-16',
        ticketDate: '2026-10-30',
        examDate: '2026-11-14',
        previewFrom: '2026-11-27',
        previewTo: '2026-12-01',
        resultDate: '2026-12-04',
      },
    ],
    scheduleUrl: 'https://www.dataq.or.kr/www/accept/schedule.do',
    fee: '50,000원. 납부방법은 신용카드·계좌이체.',
    refund: [
      { when: '접수기간 마감일 17:59:59까지', rate: '전액' },
      { when: '접수기간 종료 ~ 검정 시행 5일 전 17:59:59', rate: '50%' },
      {
        when: '검정 시행 5일 전 18:00:00 이후',
        rate: '불가',
        note: '시험일이 토요일이면 그 주 월요일 18:00:00 이후, 일요일이면 화요일 18:00:00 이후 불가',
      },
    ],
    refundNote:
      '환불 처리 기준은 시스템의 처리 완료 시간을 따른다. 입원·사망·전염병 확진·격리·군인 휴가금지·천재지변 등 예외 사유는 관련 증빙서류를 시험일로부터 30일 이내에 Q&A로 ' +
      '접수하면 100% 환불된다.',
    validity:
  ...
  },
  {
    ...
    employment: 4,
    employmentBasis: '국가공인 민간자격이지만 공인된 등급은 Associate 하나입니다.',
    whatItMeasures:
    ...
    fee:
      '전부 VAT 포함 금액이다. AICE Future 3급 30,000원 / 2급 40,000원 / 1급 50,000원, Junior 50,000원, Basic 50,000원, ' +
      'AICE Generative 2급 40,000원 / 1급 60,000원, Associate 80,000원, Professional 120,000원. 결제는 신용·체크카드, ' +
      '네이버페이, 카카오페이가 가능하고 5만 원 이상은 할부가 되지만 무이자 정책은 카드사별로 다르다. 100% 할인 쿠폰으로 결제한 경우 취소가 불가하고 미응시 시 \'미응시 ' +
      '처리\'로 전환된다. 자격증과 디지털 배지 발급은 합격 후 횟수·비용 제한 없이 무료다.',
    refund: [
      { when: '접수기간 중 또는 시험일 8일 전까지', rate: '전액' },
      { when: '시험일 7~5일 전', rate: '50%' },
      { when: '시험일 4일 전 이후', rate: '불가' },
    ],
    refundNote: 'FAQ가 밝힌 기준이고, 쿠폰으로 결제한 경우 실 결제금액을 기준으로 환불한다.',
    validity:
  ...
  },
  {
    ...
    employment: 3,
    employmentBasis: 'AWS 어소시에이트 등급이고 AWS 스택은 국내 채용에 흔합니다.',
    whatItMeasures:
  ...
  },
  {
    ...
    employment: 3,
    employmentBasis: '구글 클라우드 프로페셔널 등급이고 공고에 AWS와 함께 오릅니다.',
    whatItMeasures:
  ...
  },
  {
    ...
    employment: 2.5,
    employmentBasis: '어소시에이트지만 입문 시험이고 국내 공고 언급을 못 찾았습니다.',
    whatItMeasures:
  ...
  },
  {
    ...
    employment: 3,
    employmentBasis: '어소시에이트 등급이고 국내 고객사가 늘어 데이터 직군에서 통합니다.',
    whatItMeasures:
  ...
  },
  {
    ...
    employment: 2.5,
    employmentBasis: '같은 등급이지만 신설이라 국내 공고 언급을 못 찾았습니다.',
    whatItMeasures:
  ...
  },
  {
    ...
    employment: 4.5,
    employmentBasis: '같은 SQL 자격의 전문가 등급이고 학점은행 12학점입니다.',
    whatItMeasures:
    ...
    schedule: [
      {
        ...
        ticketDate: '2026-02-20',
        examDate: '2026-03-07',
        previewFrom: '2026-03-27',
        previewTo: '2026-03-31',
        resultDate: '2026-04-03',
        documentFrom: '2026-04-03',
        documentTo: '2026-04-10',
      },
      {
        round: '제55회',
        applyFrom: '2026-07-20',
        ...
        ticketDate: '2026-08-07',
        examDate: '2026-08-22',
        previewFrom: '2026-09-11',
        previewTo: '2026-09-15',
        resultDate: '2026-09-18',
        documentFrom: '2026-09-18',
        documentTo: '2026-09-30',
      },
    ],
    scheduleUrl: 'https://www.dataq.or.kr/www/accept/schedule.do',
    fee:
      '검정수수료 100,000원 (SQLD는 50,000원, DAP 100,000원과 동일 수준). 납부방법은 신용카드·계좌이체. 합격 후 받는 자격증은 상장형(국가공인자격)이라 ' +
      '홈페이지를 통해 즉시 발급되고 발급 수수료가 따로 없다 — 시행처가 안내하는 6,200원(발급 수수료 3,200원 + 우체국 등기배송료 3,000원)은 카드형 자격증인 ' +
      '국가기술자격 빅데이터분석기사에만 붙는다.',
    refund: [
      { when: '접수 기간 마감일 17:59:59까지', rate: '전액' },
      { when: '접수 마감 이후 ~ 검정 시행 5일 전 17:59:59', rate: '50%' },
      {
        when: '검정 시행 5일 전 18:00:00 이후',
        rate: '불가',
        note: '시험일이 토요일이면 그 주 월요일 18:00, 일요일이면 그 주 화요일 18:00 이후 불가',
      },
    ],
    refundNote:
      '접수한 시험을 다음 회차로 연기할 수는 없다. 본인 입원·직계 사망·법정 격리 전염병 확진·군인 외출 금지·천재지변 등 여섯 사례는 증빙 제출 시(시험일로부터 30일 이내 ' +
      'Q&A 접수) 100% 환불하는 예외규정이 있다.',
    validity:
      '자격 취득 후 유효기간은 합격일로부터 2년이며, 보수교육을 이수하면 영구로 전환된다. (SQLD도 동일한 규칙이다.) 보수교육은 합격일로부터 1년 ' +
      '6개월이 지난 시점부터 2년이 되기 전에 이수해야 하고, 그 기간 안에 이수하지 않으면 이수를 마칠 때까지 자격의 효력이 일시 정지된다. 이수는 ' +
      '홈페이지 마이페이지의 보수교육에서 무료로 한다.',
    prerequisite:
    ...
    notes:
      ...
      '- 회차별 통계는 여전히 공개되지 않지만 연도별 통계는 있다. 시행처 FAQ가 통계 소재를 한국직업능력연구원 민간자격정보서비스(pqi.or.kr)로 ' +
      '지정하고, 그곳 「SQL」(등록번호 2011-0771) 상세 화면에 등급별 취득현황이 연도별로 실린다. 전문가(공인) 등급은 최근 세 해 모두 검정 ' +
      '2회로 찍혀 cadence의 연 2회가 실적으로도 뒷받침된다. 다만 pqi가 「자격발급기관에서 직접 입력」한 값이라는 단서를 달아 두었다.\n' +
      '- 문제와 정답은 모두 비공개다. 시행처가 공공기관의 정보공개에 관한 법률 제9조 제1항 제5호와 자체 「데이터 자격검정 관리운영 규정」 ' +
      '제29조를 근거로 든다. 시험문제는 저작권법 보호 대상이고 기출문제 복제·배포·전송과 영리 목적 사용을 허가하지 않으며, 문제를 복원해 게시하면 ' +
      '형사처벌될 수 있다고 밝힌다 — 시험 노트에 기출을 옮기지 않는 우리 규칙과 같은 자리다.',
    unknowns: [
  ...
  },
  {
    ...
    employment: 2,
    employmentBasis: 'Microsoft가 Fundamentals로 분류한 입문 등급입니다.',
    whatItMeasures:
    ...
    notes:
      ...
      '필요)에서 제공한다.\n' +
      '- 재응시 규정은 role-based·specialty·fundamentals 시험에 공통으로 적용되므로 AI-901도 그대로 해당한다. 첫 응시에서 ' +
      '떨어지면 24시간을 기다려야 다시 볼 수 있고, 그 다음부터는 시도 사이마다 14일이 붙는다. 같은 시험은 첫 응시일로부터 12개월 안에 다섯 번까지만 ' +
  ...
  },
  {
    ...
    cadence:
      ...
      '사라지고 복구되지 않는다고 명시한다. 재응시는 첫 시도 실패 후 24시간이 지나면 가능하고, 그 다음부터는 시도 사이마다 14일을 기다려야 한다. 같은 ' +
      '시험은 첫 응시일로부터 12개월 안에 다섯 번까지만 볼 수 있고, 다섯 번 모두 떨어지면 첫 응시일로부터 12개월이 지나야 다시 볼 수 있다. 합격한 시험은 ' +
    ...
    notes:
      ...
      'AI-103이 그 대체 자격증이라는 것은 시행처가 표로 못 박았다 — AI-102 페이지의 은퇴 경고가 거는 Microsoft Skills Hub 블로그 글의 ' +
      '은퇴 표가 \'Replacement Certification\' 열에 Azure AI Apps and Agents Developer Associate (Exam ' +
      'AI-103)를 적고, 같은 글의 전환 안내 표도 \'Exam AI-102 → Exam AI-103\'으로 짝짓는다.\n' +
      '- 이 자격증을 얻는 데 필요한 시험은 AI-103 하나뿐이다 — 시행처 발표 글이 "To earn this new Certification, you need ' +
      'to pass Exam AI-103"이라고 적는다.\n' +
      '- 랩(lab) 문항이 실제로 나오는지는 여전히 공개되지 않지만, 시험 시간 120분은 시행처 시험 시간 표에서 \'랩이 포함될 수 있는 어소시에이트·엑스퍼트 ' +
      '역할 기반 시험\' 칸의 값이다(랩이 없는 같은 등급은 100분이고 은퇴한 AI-102가 100분이었다). 즉 랩이 들어갈 수 있는 유형에 속한다 — ' +
      'Microsoft는 Azure 장애 등으로 랩을 언제든 뺄 수 있어 랩 포함 시험 목록을 공개하지 않는다고 밝힌다.\n' +
