...
describe('뉴스 데이터', () => {
   ...
   * 있으면 값을 올리지 않는 것이 규칙입니다)까지 잡지는 않습니다.
   */
  it('globalNewsUpdatedAt이 최신 발표에서 14일 넘게 뒤처지지 않는다', () => {
    const newest = newsItems.map((item) => item.publishedAt).reduce((a, b) => (a > b ? a : b));
    const lagDays = Math.round((Date.parse(newest) - Date.parse(globalNewsUpdatedAt)) / 86_400_000);
    const where = `체크포인트 ${globalNewsUpdatedAt}, 최신 발표 ${newest}`;
    expect(lagDays, where).toBeLessThanOrEqual(14);
    expect(lagDays, where).toBeGreaterThanOrEqual(0);
  });
});
