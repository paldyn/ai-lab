...
마지막 갱신: 2026-09-25 (분량 상한을 걷어내고 세는 법을 산문으로 맞춤 — 남아 있던
「초급 5,000~9,000자」 탓에 그날 실행이 연습 문제 여섯 개를 뺐다)
이전 갱신: 2026-09-23 (node_modules가 없으면 npm test가 검사 없이 0을 내는 것을
검증 절에 적음 — 그날 실행이 기준 테스트를 초록으로 잘못 읽었다)
이전 갱신: 2026-09-22 (줄을 지웠으면 ARTICLE-DEPTH-PLAN.md 머리말도 고친다고 적음 —
09-19·09-22에 빠뜨려 편수가 갈렸다)
이전 갱신: 2026-09-07 (신규를 멈추고 보강 4편으로 돌림 — 아래 「지금은 새 글을 쓰지 않는다」)
...
시작하고 거기로 돌아와 끝난다. 분량은 **산문 6,000자 이상**이다(세는 법은 아래 D절).
...
- **분량은 중급·고급과 같이 산문 6,000자 이상**이다(세는 법은 아래 D절). 상한은 없다.
...
**줄을 지웠으면 `ARTICLE-DEPTH-PLAN.md`의 머리말도 함께 고친다.** 지금 맨 위의
「마지막 갱신」을 「이전 갱신」으로 내리고 그 자리에 오늘 줄을 새로 쓴다. 꼴은 그대로다:

```
마지막 갱신: <오늘(KST)> (math-for-ai 넷을 채우고 지움 — N편 남음.
math-for-ai만 세면 M편이다. 목록의 항목을 직접 센 값이다)
```

N·M은 **지운 뒤에 직접 센다** — 앞 줄의 수에서 넷을 빼지 마라. 같은 날 다른 루틴이
먼저 지웠으면 그만큼 더 줄어 있다.

```bash
grep -c '^- `' ARTICLE-DEPTH-PLAN.md
sed -n '/^### math-for-ai$/,/^### /p' ARTICLE-DEPTH-PLAN.md | grep -c '^- `'
```

**이 줄을 안 쓰면 머리말의 편수가 실제와 갈린다.** 09-19와 09-22에 이 루틴이 넷을
지우고 줄을 안 남겨, 뒤이어 돈 AI 글 루틴이 머리말을 열었을 때 적혀 있던 수가 실제보다
넷 많았다. 두 번 다 AI 루틴이 제 줄에 「같은 날 수학 루틴이 먼저 넷을 지워 …이던
자리다」로 바로잡았지만, **AI 루틴이 뒤에 안 도는 날이면 틀린 수가 그대로 남는다.**
09-20·09-21에는 이 루틴이 제대로 썼으니 규칙이 어려운 것이 아니라 적혀 있지 않았을
뿐이다.
...
- 분량은 세 트랙 모두 **산문 6,000자 이상**이다. **상한은 없다.**

  **세는 법을 하나로 못박는다** — 보강 절에 적은 것과 같은 명령이다. frontmatter를 뺀
  본문에서 **코드 펜스·수식·그림 줄을 빼고** 공백을 제외한 글자 수다. `CLAUDE.md`의
  「학습 글의 산문은 6,000자 이상이다」와 같은 단위다.

  ```bash
  python3 - "src/content/articles/<slug>.md" <<'EOF'
  import re, sys
  t = open(sys.argv[1]).read().split('---', 2)[-1]
  t = re.sub(r'```.*?```', '', t, flags=re.S)
  t = re.sub(r'\$\$.*?\$\$', '', t, flags=re.S)
  ...
  **전체 글자로 세지 마라.** 2026-09-15까지는 「초급 5,000~9,000자, 중급 6,000~12,000자,
  고급 8,000~15,000자」를 공백만 뺀 전체 글자로 쟀다. 수학 글은 수식이 본문의 절반
  가까이라, 산문이 6,000자를 겨우 넘긴 정상적인 초급도 전체로는 8,000~14,700자가 된다.
  상한이 지시서에 남아 있던 탓에 2026-09-25 실행이 산문 6,213자짜리 초급 한 편을
  9,000자 밑으로 줄이려고 수식을 풀어 쓰고 **연습 문제 여섯 개를 뺐다** — 이 트랙의
  본체인 손계산을 분량 숫자 때문에 버린 것이다.

  **상한 대신 절당 분량을 본다.** 산문이 길어도 절이 많아 절당 400자 밑으로 내려가면
  그건 길어서가 아니라 잘게 쪼갠 것이다(`CLAUDE.md`의 「절을 늘리기 전에 절당 분량을
  재라」). 줄일 것은 연습 문제나 수식이 아니라 절 수다.
...
⚠ **`node_modules`가 없으면 `npm test`가 아무것도 검사하지 않고 종료 코드 0을 낸다.**
`pretest`의 `sync-techblog.mjs`가 `gray-matter`를 못 찾아 죽는데, npm이 그 실패를 삼켜
셸에는 0이 돌아온다 — 로그를 안 읽으면 「초록」으로 보인다. 컨테이너를 새로 받으면
의존성이 안 깔려 있으므로 **검증 전에 한 번 확인한다.**

```bash
[ -d node_modules ] || npm ci
npm test 2>&1 | tail -8      # "Tests N passed" 줄을 눈으로 확인한다
```

2026-09-23 보강 실행이 이걸 밟았다. 시작할 때 돌린 기준 `npm test`가 0을 내서 통과로
읽었는데, 실제로는 한 줄도 안 돌고 있었다.
