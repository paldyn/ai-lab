...
시작하고 거기로 돌아와 끝난다. 분량은 **중급 6,000~12,000자, 고급 8,000~15,000자**
(세는 법은 아래 D절).
...
- **분량은 5,000~9,000자**다(세는 법은 아래 D절). 중급보다 짧다.
...
**계획도 진도도 작업 트리에서 읽는다** — 계획은 `src/data/curriculum.ts`이고 어디까지
썼는지는 `src/content/articles/`다. 클론이 뒤처져 있으면 이미 쓴 편을 안 쓴 것으로 읽어
같은 편을 다시 쓴다.
...
```bash
while read -r slug; do grep -n -A2 "\`$slug\`" MATH-PLAN.md; done < /tmp/next_slugs.txt
```
...
2. **제목** — 그 절이 다루는 것의 **이름**이다. 설명하지 말고 이름을 적는다
   (「왜 여기서 근호가 사라지는가」도 「근호가 사라지는 자리」도 아니라 **「분모의 유리화」**).
...
고쳐 적는다** — 다음 날 그 줄이 다시 잡혀 이어 채운다. 2026-09-08에 세 줄이 5,139·
5,229·5,662자에서 지워졌다. 한 편에 다 못 채우는 글이 있는 것은 괜찮지만, 안 채운
것이 끝난 것으로 기록되면 그 글은 영영 안 돌아온다.
...
**열어 보고 채울 거리가 없으면 억지로 늘리지 않는다** — 그때만 6,000자에 못 미쳐도
줄을 지우고 보고에 「채울 거리 없음」으로 적는다. 이것은 예외이지 기본이 아니다.
...
- **pubDate**: `TZ='Asia/Seoul' date +%Y-%m-%d`
  ...
  **실행 시각 안내에 적힌 UTC 날짜를 그대로 쓰지 마라.** 이 루틴은 20:00 UTC에
  도는데 그 시각은 KST로 **다음 날** 05:00이라 UTC 날짜는 늘 하루 이르다.
  2026-09-07 실행이 다섯 편을 전부 `2026-09-06`으로 적었다 — `TODAY` 변수를 쓰는
  커밋 제목은 「(2026-09-07)」로 맞게 나갔으니 frontmatter만 손으로 적은 것이다.
  **위 명령을 실제로 돌려 그 출력을 붙여 넣는다.** 어긋나도 사슬 검사에는 안
  걸린다(앞 글도 같은 날이라 통과한다) — 대신 홈의 「TODAY'S UPDATES」와 최신 글에서
  그날 나간 다섯 편이 통째로 어제 것이 된다.
- 분량은 트랙에 따른다 — **초급 5,000~9,000자, 중급 6,000~12,000자,
  고급 8,000~15,000자.**

  **세는 법을 하나로 못박는다** — frontmatter를 뺀 본문에서 **공백만 제외한** 글자 수다.
  코드 블록과 수식과 그림 줄은 **센다**. 실행마다 다른 방식으로 세면 같은 글이
  6,063자도 되고 6,612자도 된다(2026-08-21 실행이 그랬다).

  ```bash
  python3 -c "import re,sys;print(len(re.sub(r'\s','',open(sys.argv[1]).read().split('---',2)[2])))" src/content/articles/<slug>.md
  ```

  중급 숫자는 실측에서 나왔다. 2026-08-22까지 쓴 중급 12편이 5,685~7,703자
  (중앙 6,933)인데 기준은 8,000자 이상이라 **한 편도 지킨 적이 없었다.** 지킬 수 없는
  숫자는 기준이 아니므로 실제에 맞춰 내리고 하한만 조금 올렸다. 고급 숫자는 아직
  한 편도 안 써 본 값이라 **첫 배치를 쓰고 나서 같은 방식으로 다시 맞춘다.**
...
- **뼈대부터 짜고 쓴다** — 절(`##`) 4~7, 절마다 소절(`###`) 2~4. 절 제목도 소절
  제목도 **그것이 다루는 것의 이름**이다(「분모의 유리화」·「최대공약수」처럼).
  설명하는 문장이나 물음으로 달지 않는다. 규칙은 `CLAUDE.md`의 「글의 뼈대」에 있다
