...
- **수학 글은 「한 줄 정의」로 연다.** 본문 맨 앞에 인용문 하나를 두고, 그 글에서
  처음 만나는 낱말 두셋을 한 줄에 하나씩 못 박는다. 프론트매터 바로 다음,
  「지난 글」 문단보다 앞이다.
  ...
  꼴은 `**낱말**은 ~입니다` 하나다. 줄 사이에는 `>` 만 있는 빈 줄을 둔다 — 없으면
  두 줄이 한 문단으로 붙는다. 화면에서는 `.article-prose > blockquote:first-child`가
  「한 줄 정의」 딱지가 붙은 상자로 그린다. **맨 앞 인용문일 때만** 걸리므로 글
  중간의 인용문은 그대로 인용문이고, 원고에서 꾸미지 않는다.

  고르는 기준은 **읽다가 멈추게 하는 말**이다. 「에라토스테네스의 체가 뭐였더라」에서
  멈춘다는 말이 있었는데, 본문 어딘가에 정의가 적혀 있어도 찾아 올라가야 하면 늦다.
  2026-08-14에 수학 30편에 붙였다. 정의는 본문이 실제로 쓰는 말과 같아야 한다.
