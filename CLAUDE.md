...
**사용법·입문 글은 그 글이 다루는 모델 쪽에 둔다.** 2026-08-18에 바꿨다 — 예전에는
수집 루틴이 「사용법·생활 팁」·「교육·입문 콘텐츠」로 묶어 통째로 건너뛰었는데,
「GPT-5.6으로 만드는 법」 같은 글을 찾는 사람이 서 있는 자리가 곧 뉴스의 'AI 모델'
탭이다. 위에서부터 물어 처음 '그렇다'가 나온 곳에 넣는다.

1. 그 글에서 **처음 공개하는 것**이 있는가 — 새 API 표면, 새 서비스 티어, 새 가격.
   → `company` / `Product`. 새로 공개한 그것이 항목의 주제이지 가이드가 주제가 아니다.
2. 특정 모델을 **어떻게 쓰는지**를 다루는가 → `kind: 'model'`. `category`는 그 모델의
   갈래를 따른다(GPT-5.6 가이드면 `Frontier`).
3. 다루는 모델이 없는 생활 팁인가(저녁 모임 차리는 법 같은 것) → 건너뛴다.
   `model`의 네 `category` 중 넣을 칸이 없다.
