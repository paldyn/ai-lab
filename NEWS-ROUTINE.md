...
받아 둔 클론이 원격보다 뒤처져 있을 수 있다. 그러면 `globalNewsUpdatedAt`이 실제보다
과거로 보이고, 이미 실려 있는 항목을 없는 것으로 읽어 다시 쓰게 된다 — 그 커밋은
push에서 막히고 rebase는 `news.ts` 한복판에서 충돌한다. 2026-09-01 실행이 그랬다.
클론의 HEAD가 나흘 전이라 체크포인트를 `2026-08-22`로 읽고 08-25치를 통째로 다시 썼다가
버렸고, 실행 시간의 절반을 여기서 썼다. **체크포인트를 읽는 것은 이 줄 다음이다.**

그다음 다섯 곳을 본다. 앞 세 곳은 블로그 RSS, Anthropic 블로그는 RSS가 없어 목록 페이지를
...
**14일보다 오래된 구멍도 RSS에는 남아 있다.** 오래 「RSS에 안 남는다」고 적어 두었는데
2026-08-24에 세어 보니 `openai.com/news/rss.xml`은 2015년까지 1,143건을 싣는 전체
아카이브였다. 사라져서 못 채우는 것이 아니라 **이 루틴의 창이 좁아서 안 보이는 것뿐이다.**

그러니 그런 구멍은 버리지 말고 보고에 URL과 발행일을 적는다. 창을 넓히는 것은 이
루틴이 할 일이 아니고, 로컬 보충 루틴(`LOCAL-NEWS-ROUTINE.md`)이 30일 바닥으로
따로 훑는다. 어느 쪽이든 **제목만 보고 지어내지 마라.**
...
- **값을 실제로 고쳤는지 커밋 전에 눈으로 확인한다.** 안 올려도 아무 데도 안 깨지고
  ...
  **이 눈 확인만으로는 세 번 샜다** — 08-17·08-18에 이어 09-10~09-14 실행이
  연달아 빠뜨려 값이 09-09에 멈춰 있었고 그 뒤 실행마다 엿새를 다시 훑었다.
  그래서 `src/data/news.test.ts`에 **14일 넘게 뒤처지면 서는 검사**를 두었다.
  그 선이 창의 상한이라 넘는 순간 앞의 날은 RSS에서 사라져 사람이 손으로 채워야
  한다. **검사가 통과한다고 값이 맞는 것은 아니다** — 하루이틀 뒤처진 것은 안
  잡히니 위 grep은 그대로 한다.
