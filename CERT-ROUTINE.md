...
「PALDYN AI Lab — 자격증 데이터 갱신 (주 1회)」이 **실행할 때마다 읽는 지시서**다.
프롬프트는 이 파일을 읽으라는 쪽지뿐이니 **여기만 고치면 된다.**
뉴스·수학·AI·리서치 루틴과 같은 방식이다.

**2026-09-07부터 이 루틴은 맥에서 돈다** — 예약 작업 `ailab-cert-refresh`,
월요일 07:00 KST. 클라우드 Routine(`trig_01WcG7uPVk2bRM1BukjjnnLj`, 월 06:30 KST)은
켜 둔 채로 두지만 **지금은 아무 일도 못 한다.** 그 샌드박스의 egress 정책이 시행처
도메인을 전부 막아서다 — `www.dataq.or.kr`·`learn.microsoft.com`·`aws.amazon.com`·
`aice.study`·`www.q-net.or.kr`·`www.nvidia.com`·`www.databricks.com` 전부 차단이고
`cloud.google.com` 하나만 열린다. 맥에서는 여덟 곳 다 200이다.

08-30·09-06 두 번의 실행이 그래서 통째로 헛돌았고 `certs.ts`는 08-27 이후 한 줄도
바뀌지 않았다. 클라우드 쪽을 끄지 않고 두는 것은 **egress가 열리는 날을 알려 주는
신호**로 쓰기 위해서다 — 그쪽이 다시 무언가 고치기 시작하면 로컬 작업을 접으면 된다.
로컬 작업은 시작할 때 원격 main에 맞추므로 둘이 같은 주에 돌아도 겹쳐 쓰지 않는다.

`---` 아래가 지시다. 위 머리말은 사람이 읽는 자리라 루틴은 건너뛴다.

마지막 갱신: 2026-09-07 (egress 차단으로 실행 자리를 맥으로 옮김)
이전 갱신: 2026-08-26 (수험표·사전점수공개·서류제출 칸을 더하고, 원문 값을 추측으로 지우지 말라고 못 박음)
...
## STEP 4-A — 환불 규정은 표로 옮긴다

공식 페이지의 응시료·환불 안내를 다시 읽었으면 환불 부분은 `refund` 배열에 담는다.
`when`은 시행처 문장의 시점 표현을 그대로 옮기고(「접수 마감 이후 ~ 검정 시행 5일 전
17:59:59」) `rate`는 `전액`·`50%`·`불가` 꼴로만 적는다. 날짜로 계산해 적지 않는다 —
회차마다 틀린다. 예외 사유(증빙 제출 시 100% 환불 같은 것)와 처리 기준 시각은
`refundNote`에 둔다.

**`fee` 문장에는 환불 이야기를 남기지 않는다.** 두 곳에 적으면 한쪽만 갱신되고,
`src/data/certs.test.ts`가 그것을 잡는다.

## STEP 4-B — 시험 구성이 바뀌면 노트도 함께 고친다

`format`(문항 수·시험 시간·합격 기준)이 바뀐 자격증은 **그 자격증의 시험 노트도 본다.**
모의고사는 실제 문항 구성을 그대로 옮겨 짜고 총정리는 과목별 문항 수를 적어 두므로,
데이터만 고치면 원고가 조용히 낡는다.

```bash
grep -rn "문항\|점 이상\|과락\|합격선" src/content/certs/<자격증 id>/ | head -20
```

바뀐 값이 박혀 있으면 그 줄을 고치고, 모의고사의 과목 비중이 실제와 어긋나면 문항을
옮긴다. 고친 파일은 보고에 적는다.
...
- `'어소시에이트지만 입문 시험이고 국내 공고 언급을 못 찾았습니다.'` (nvidia-genai-llm)
- `'같은 등급이지만 신설이라 국내 공고 언급을 못 찾았습니다.'` (databricks-genai-associate)

**예로 든 문장이 데이터에 실제로 있는지 먼저 확인한다.** 2026-09-07까지 여기
적혀 있던 ai-103의 「아직 베타라」 문장은 데이터에서 이미 사라진 지 오래였고,
그 회차 조사가 없는 문장을 「무효」로 판정하느라 한 칸을 헛돌았다. STEP 2의
grep이 뽑아 준 실제 줄만 대상으로 삼는다.
...
푸시는 도는 자리에 따라 다르다. 맥에서는 자격 증명이 이미 있다.
