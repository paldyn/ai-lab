...
4. 나란히 놓을 줄은 목록으로 적거나 앞 줄 끝에 역슬래시(`\`)를 붙인다 — 문단 안의
   홑 줄바꿈은 공백이 되어 네 보기가 한 줄로 이어 붙는다. 객관식 보기가 쓰는 그 방식이다.
5. 낱말은 처음 나오는 자리에서 한 문장으로 정의하고 계속 간다. **낱말끼리 얹혀
   있으면 관계를 같은 문장에서 말한다** — 따로 정의해 늘어놓으면 나란한 것들처럼
   읽혀 오히려 나빠진다. 표 칸의 괄호, 절 제목, 첫 등장보다 뒤의 정의, 아직 안 나온
   낱말로 지금 낱말을 정의하는 것은 짚은 것이 아니다.
6. 굵게는 낱말을 처음 이름 붙이는 자리에만 쓴다. 주장 문장 전체, 표·목록의 항목
   이름, 이미 앞에서 이름 붙인 낱말에는 쓰지 않는다 — 1,000자당 두셋이면 넉넉하다.
7. 코드 펜스에는 언어명을 붙인다.
