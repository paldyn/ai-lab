...
마지막 갱신: 2026-09-07 (뼈대 기준을 학습 글과 맞춤 — 절 4~7, 소절 2~4, 명사구 제목)
...
쪼갠 목록이고, 개념 주제 406편과 모의고사 71편으로 **모두 477편**이다.
...
| 번호대 | 무엇 | `kind` |
| --- | --- | --- |
| `01`~`79` | 계획의 개념 주제. **파일 번호 = 계획의 몇 번째 주제인가** | `개념` |
| `80`~`89` | 과목 총정리. **루틴은 여기에 쓰지 않는다** | `개념` |
...
1. 절은 `##`, 소절은 `###`. **`#`은 쓰지 않는다** — 제목은 frontmatter에 있다.
   **절 4~7, 절마다 소절 2~4다.** 절만 늘어놓고 소절이 없으면 목차가 평평해져
   어디까지 읽었는지 안 보인다. 절 제목과 소절 제목은 차례처럼 읽히는 **명사구**로
   단다 — 「왜 정규화를 하는가」가 아니라 「정규화가 푸는 문제」다. 기준은
   `CLAUDE.md`의 「글의 뼈대」에 있고 학습 글과 같다.
...
**총정리(80번대)는 다르다.** 계획의 개념 노트 여럿을 한자리에서 되짚는 글이라
**5,000~9,000자**로 쓰고(덮는 주제가 열여덟인 편도 있고 코드가 든 편은 더 길어진다),
그 과목의 계획 주제를 **하나도 빠뜨리지 않고 계획 순서대로**
훑는다. 설명을 새로 펼치는 자리가 아니라 「무엇이 어디에 있었는지」를 되살리는 자리이므로
주제마다 한 문단이나 표 한 줄로 압축한다. **아직 없는 노트에 링크하지 않는다** —
`npm test`가 내부 링크 존재를 검사한다. 제목만 텍스트로 적는다.
...
**없는 유형을 만들어 넣지 않는다.** ADsP는 시행처가 「총 문항수 : 50문항 - 객관식
50문항」으로 못 박아 단답형이 없다(a_06.do). 수험서에 단답형이 있다는 말이 돌지만
시행처 문서에는 그 낱말이 한 번도 안 나온다. 유형을 섞기 전에 시행처 표를 먼저 연다 —
주관식이 있는 시험은 ADP처럼 표에 열이 따로 서 있다.
