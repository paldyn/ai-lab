...
학습 글 534편의 산문을 전부 세어 만든 작업 큐이고, 합치기 57짝과 채우기 96편이
카테고리별로 적혀 있다.
...
출력된 섹션에서 **위에서부터 네 항목**을 집는다. **합치기가 남아 있으면 합치기가
...
- **뼈대부터 짜고 쓴다** — 절(`##`) 4~7, 절마다 소절(`###`) 2~4.
  제목은 차례처럼 읽히는 **명사구**다(「왜 초기화가 중요한가」가 아니라
  「초기화가 정하는 것」). 규칙은 `CLAUDE.md`의 「글의 뼈대」에 있다.
