...
「PALDYN AI Lab — 리서치 글 보강 (하루 2편)」 Routine(`trig_01CNduMiXKy1f4Rg7a2Deo9m`)이
...
마지막 갱신: 2026-09-07 (신규를 멈추고 보강 2편으로 돌림 — 아래 「지금은 새 글을 쓰지 않는다」)
이전 갱신: 2026-09-07 (하루 2편 신규에서 신규 1 + 보강 1로 바꿈)
...
**2026-09-07부터 이 루틴은 하루 2편을 「보강」한다. 새 글은 한 편도 쓰지 않는다.**

리서치 44편은 분량이 문제가 아니다 — 산문 중앙값이 5,320~5,651자로 학습 글의 두 배다.
문제는 **절이 아홉**이라는 것이다(기준은 4~7). 실험을 새로 돌리기 전에 있는 글의 층을
세운다.

그래서 **STEP 1~4(주제 결정·실험·집필·자기검사)는 지금 건너뛴다.** 지우지 않고 남겨
두는 것은 보강이 끝나면 그날 다시 켤 자리이기 때문이고, `RESEARCH-PLAN.md`도 그대로 둔다.
...
작성 대상은 GitHub Trees API로 고르지만 **계획(`RESEARCH-PLAN.md`)과 그 칸의 사슬
꼬리는 작업 트리에서 읽는다** — 클론이 뒤처져 있으면 꼬리가 아닌 글에 이어 붙어
본문과 화면의 이동 칸이 서로 다른 글을 가리킨다.
...
7. **마무리 블록** — 본문 맨 아래에 이 순서로 둔다.
   ...
   **다음 글:** [{제목}](/articles/{next-slug})
   ```
   '지난 글'은 **같은 `category`에서 가장 최근에 쓴 글**이다 — 주제가 가까운 옛 글이 아니라 **사슬의 꼬리**, 즉 그 칸에서 `pubDate`가 가장 늦고 마무리 블록에 '다음 글' 줄이 없는 글이다. 리서치는 `lab-notes`·`paper-notes`·`tools` 세 칸이라 칸을 건너뛰어도 안 된다.

   화면 아래 이동 칸이 사슬이 아니라 **분야 안의 번호**로 앞뒤를 찾기 때문이다. 칸을 건너뛰거나 꼬리가 아닌 옛 글에 이으면 본문과 이동 칸이 서로 다른 글을 가리킨다 — 2026-08-19에 칸을 건너뛴 글 21편을 고쳤고, 2026-08-21에 `lab-dimension-vs-corpus-size`가 주제가 가까운 13편 전 글에 이어 같은 어긋남이 하나 났다. **`npm test`는 이것을 못 잡는다** — 사슬만 보면 성립하는 링크라서다. 쓰는 사람이 지켜야 한다.

   그러니 링크를 적기 전에 꼬리를 **계산해서** 확인한다.

   ```bash
   # 그 칸의 글을 pubDate 순으로 세우고, '다음 글' 줄이 없는 맨 끝이 꼬리다
   for f in $(grep -l 'category: "lab-notes"' src/content/articles/*.md); do
     echo "$(grep -m1 '^pubDate:' "$f" | cut -d'"' -f2) $(basename "$f" .md) $(grep -c '^\*\*다음 글:' "$f")"
   done | sort
   ```

   그 칸에 먼저 쓴 글이 없으면 이 줄을 아예 쓰지 않는다. **'다음 글'은 이 글에 적지 않고**
   앞 글의 파일을 열어 그 마무리 블록에 이번 글을 가리키는 줄을 더한다.
   그 '다음 글' 줄을 빠뜨리면 화면에 아무 표시가 안 나는데, 그쪽은 `npm test`가 사슬과
   대조해 잡는다(위에서 못 잡는다고 한 것은 꼬리가 아닌 글에 이은 경우다).
...
쓰고, 작성 전에 `cat SVG-STYLE.md`를 읽는다.
...
## STEP 5 — 보강 2편 (여기서 시작한다)

**이미 있는 리서치 글 두 편의 뼈대를 손본다.** 절이 많은 쪽부터다.
...
「캐시가 빗나가는 자리」도 아니라 **「캐시 적중률」**). 보통 두 자에서 열 자 사이이고,
「~라는 것」·「~하는 자리」·「~인 이유」 같은 꼬리를 붙이지 않는다.
기준은 `CLAUDE.md`의 「글의 뼈대」에 있다.
...
**묶기가 끝나면 굵게를 센다. 세기 전에는 그 글을 놓지 않는다.**
굵게는 낱말을 처음 이름 붙이는 자리에만 쓴다 — 주장 문장 전체에 쓰지 말고,
표·목록의 항목 이름에도 쓰지 말고(표와 목록은 그 자체가 구조다), 이미 앞에서 이름
붙인 낱말을 뒤에서 또 굵게 하지 않는다. 본문이 다 주장인데 그중 몇을 골라 굵게 하면
나머지가 덜 중요한 말처럼 읽힌다. **1,000자당 두셋이면 넉넉하다** — 466편의 중앙값이
1,000자당 7.3개이고, 2026-09-10에 한 편을 101개에서 28개로 줄였다.

세는 것은 이 한 줄이다(코드 펜스 안의 `**kw` 같은 파이썬 문법을 빼고 센다).
...
o=[x for x in re.split('('+FENCE+'.*?'+FENCE+')',t,flags=re.S) if not x.startswith(FENCE)]
b=[m for x in o for m in re.findall(r'\*\*(.+?)\*\*',x,re.S)]
p=t.split('---',2)[2]
p=re.sub(FENCE+'.*?'+FENCE,'',p,flags=re.S)
p=re.sub(r'\\\$\\\$.*?\\\$\\\$','',p,flags=re.S)
n=len(re.sub(r'\s','',p))
print(f'{len(b)}개 · {len(b)/n*1000:.1f}/천자')
[print(' -',m[:40]) for m in b]
" src/content/articles/<슬러그>.md
````

**남은 굵게를 차례로 읽었을 때 그 글의 용어 목록이 되어야 한다.** 안 되면 아직 덜
푼 것이다. 2026-09-17에 이 루틴이 두 편의 뼈대만 고치고 굵게를 28→28·29→29로
그대로 두고 갔다 — 6.5/천자였다. 사람이 뒤에 4·4로 줄였다.

리서치 글은 값이 표와 터미널 출력에 있어 남는 굵게가 지표·기법 이름 서넛뿐인 것이
정상이다. 적다고 억지로 채우지 않는다.
...
git commit -m "post: 리서치 글 보강 2편 ($TODAY)"
...
- **보강한 글 둘**: 절을 몇 개에서 몇 개로 줄였고 어떤 절을 어느 절 아래로 내렸는지
