...
수학은 앞 글이 뒤 글의 전제가 되므로 날짜가 아니라 `src/data/curriculum.ts`가
정한 순서로 보여 줍니다 — 같은 날 여러 편이 나가 `pubDate`만으로는 하루 안의
순서가 잡히지 않습니다. 목록은 쓰는 순서(초급 → 중급 → 고급)의 역순이라
나중에 쓴 글이 위로 옵니다. 수학 글을 추가하면 그 목록에도 슬러그를 넣어야 합니다.
...
검색 진입점은 하나입니다. 헤더 돋보기 또는 `/` 키로 오버레이가 열리고, 그 안에서
범위(전체·개념·리서치·뉴스)를 골라 제목·요약·태그를 훑습니다. 목록 페이지에는
검색창을 두지 않습니다.

색인을 따로 만들지 않습니다. 글 메타데이터가 이미 `virtual:article-index`로
번들에 들어 있어 추가로 받을 것이 없습니다. 점수는 제목 시작 > 제목 포함 >
태그 일치 > 태그 포함 > 요약 포함 순이며 `src/lib/search.ts`에 있습니다.

목록 페이지의 태그 칩은 자주 쓰인 상위 14개만 보여 줍니다. 태그가 1,200개를
넘어 전부 깔면 필터가 본문보다 길어지고, 그중 대부분은 한 편에만 붙은 것이라
필터 구실을 못 합니다. 나머지는 검색이 맡습니다.
